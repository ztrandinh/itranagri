...
export async function proxy(req: NextRequest) {
  ...
  const pub = pathname === "/login" || pathname.startsWith("/api/auth/login") || pathname.startsWith("/trace") || pathname.startsWith("/api/public") || pathname.startsWith("/_next") || pathname === "/manifest.webmanifest" || pathname === "/sw.js" || /\.(png|svg|ico|jpg|webp)$/.test(pathname);
