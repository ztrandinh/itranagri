...
export async function proxy(req: NextRequest) {
  ...
  if (pathname.startsWith("/api/exports/") && limited(`exports:${ip}`, 30)) return NextResponse.json({ error: "ERR_RATE_LIMIT" }, { status: 429 });
  if (pathname.startsWith("/api/upload") && limited(`upload:${ip}`, 120)) return NextResponse.json({ error: "ERR_RATE_LIMIT" }, { status: 429 });
  if (pathname.startsWith("/api/jobs/") && limited(`jobs:${ip}`, 60)) return NextResponse.json({ error: "ERR_RATE_LIMIT" }, { status: 429 });
