...
export async function POST(req: Request, { params }: { params: Promise<{ table: string }> }) {
  ...
  try {
    return await withCtx(s, async (c) => {
      ...
      if (!exists) {
        if (!pk) { if (t.codePrefix) { const code = (await c.query("select next_code($1,$2) as c", [s.farmId, t.codePrefix])).rows[0].c; pk = String(code); data[t.pk] = pk; } else if (cols.find((x) => x.name === t.pk)?.type === "uuid" || cols.find((x) => x.name === t.pk)?.type === "integer") { delete data[t.pk]; } else return NextResponse.json({ error: "ERR_PK_REQUIRED", detail: `Cần nhập ${t.pk}` }, { status: 400 }); }
