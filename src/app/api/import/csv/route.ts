    ...
    return await withCtx(s, async (c) => {
      ...
      if (t) {
        ...
        for (let i = 0; i < rows.length; i++) {
          ...
          if (!data[t.pk]) { if (t.codePrefix) data[t.pk] = (await c.query("select next_code($1,$2) as c", [t.farmScoped ? s.farmId : "HQ", t.codePrefix])).rows[0].c; else if (!["uuid", "integer"].includes(cols.find((x) => x.name === t.pk)?.type ?? "")) { errors.push({ row: i + 1, error: `Thiếu ${t.pk}` }); continue; } }
