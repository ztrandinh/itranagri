...
export async function POST(req: Request, { params }: { params: Promise<{ job: string }> }) {
  ...
  for (const f of farms) {
    ...
    if (job === "gsdaily" || job === "all") { out[`gs_omissions:${f}`] = (await adminPool().query("select gen_gs_omissions($1, current_date-1) as n", [f])).rows[0].n; }
    ...
    if (job === "tasks" || job === "all") { out[`deleg_end:${f}`] = (await adminPool().query("select end_delegations($1) as n", [f])).rows[0].n; out[`stale_closed:${f}`] = (await adminPool().query("select expire_stale_tasks($1) as n", [f])).rows[0].n; out[`tasks:${f}`] = (await adminPool().query("select itran_generate_tasks_v2($1) as n", [f])).rows[0].n; out[`monitor:${f}`] = (await adminPool().query("select gen_monitoring_tasks($1) as n", [f])).rows[0].n; out[`compliance:${f}`] = (await adminPool().query("select gen_compliance_tasks($1) as n", [f])).rows[0].n; out[`recording:${f}`] = (await adminPool().query("select gen_recording_alerts($1) as n", [f])).rows[0].n; out[`amu:${f}`] = (await adminPool().query("select gen_amu_alerts($1) as n", [f])).rows[0].n; out[`mortality:${f}`] = (await adminPool().query("select gen_mortality_alerts($1) as n", [f])).rows[0].n; out[`herd:${f}`] = (await adminPool().query("select gen_herd_actions($1) as n", [f])).rows[0].n; out[`cyclecount:${f}`] = (await adminPool().query("select gen_cycle_counts($1) as n", [f])).rows[0].n; out[`dunning:${f}`] = (await adminPool().query("select run_dunning($1) as n", [f])).rows[0].n; out[`supervision:${f}`] = (await adminPool().query("select run_supervision_auto($1) as n", [f])).rows[0].n; if (new Date().getDay() === 1) { out[`training:${f}`] = (await adminPool().query("select gen_training_week($1) as n", [f])).rows[0].n; out[`sup_tasks:${f}`] = (await adminPool().query("select gen_supervision_tasks($1) as n", [f])).rows[0].n; await adminPool().query("select run_supervision_auto($1, (date_trunc('week', current_date) - interval '7 days')::date)", [f]); } await adminPool().query("select refresh_compliance_gaps()"); out[`assigned:${f}`] = (await adminPool().query("select assign_open_tasks($1) as n", [f])).rows[0].n; }
