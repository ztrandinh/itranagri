  ...
  try {
    const out = await withCtx(s, async (c) => {
      switch (b.action) {
        ...
        case "qc_hold": { if (!["owner","director","tech_head","team_lead","auditor","it_engineer"].includes(s.role)) throw new Error("ERR_FORBIDDEN_ROLE"); const r = await c.query("select qc_hold($1,$2,$3,$4,$5,$6) as id", [s.farmId, b.ref_type ?? "LOT", b.ref, b.reason, b.severity ?? "TRUNG", s.staffId]); return { ok: true, id: r.rows[0].id }; }
        ...
        case "recall_lot": { if (!["owner","director","tech_head","auditor"].includes(s.role)) throw new Error("ERR_FORBIDDEN_ROLE"); if (!b.lot_id || !b.reason) throw new Error("ERR_MISSING"); const r = await c.query("select recall_lot($1,$2,$3,$4,$5) as id", [s.farmId, b.lot_id, b.reason, s.staffId, b.severity ?? "NANG"]); return { ok: true, recall_id: r.rows[0].id }; }
        case "record_intake": { if (s.role === "auditor") throw new Error("ERR_FORBIDDEN_ROLE"); if (!b.sku || !b.qty) throw new Error("ERR_MISSING"); const r = await c.query("select record_intake($1,$2,$3,$4,$5,$6,$7,coalesce($8::date,current_date),$9,$10,$11) as j", [s.farmId, b.sku, Number(b.qty), s.staffId, b.source_type ?? null, b.source_id ?? null, b.client_ref ?? null, b.date ?? null, b.grade ?? null, b.unit ?? null, b.warehouse ?? null]); return { ok: true, ...r.rows[0].j }; }
        ...
        case "record_reading": { const r = await c.query("select record_reading($1,$2,$3,$4,$5,$6,$7,$8,'APP') as res", [s.farmId, b.metric_id, Number(b.value), b.ts ?? null, b.paper_serial ?? null, b.note ?? null, s.staffId, b.client_ref ?? `rd-${b.metric_id}-${Date.now()}`]); return { ok: true, ...(r.rows[0].res as object) }; }
