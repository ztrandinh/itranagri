...
export function Search({ big }: { big?: boolean }) {
  ...
  return (<div className={`relative flex items-center gap-1 ${big ? "" : "hidden md:flex"}`}><QrScan className={big ? "btn-secondary !py-2 !px-3" : "rounded-lg border border-slate-300 px-2 py-1 text-sm"} onResult={(v) => { setQ(v); }} />
    <input className={big ? "input !text-lg" : "rounded-lg border border-slate-300 bg-slate-50 px-3 py-1.5 text-sm w-48 lg:w-72 focus:outline-none focus:ring-2 focus:ring-emerald-500"} placeholder="🔍 Tìm bò, ô, SKU, người…" aria-label="🔍 Tìm bò, ô, SKU, người…" value={q} onChange={(e) => setQ(e.target.value)} onFocus={() => hits.length && setOpen(true)} onBlur={() => setTimeout(() => setOpen(false), 200)} onKeyDown={(e) => { if (e.key === "Enter" && hits[0]) location.href = `/xem/${hits[0].type}/${encodeURIComponent(hits[0].id)}`; }} />
