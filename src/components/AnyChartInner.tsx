...
export default function AnyChart(p: AnyChartProps) {
  ...
  return (
    <div>
      ...
      </Ch></ResponsiveContainer> : <div className="h-full flex items-center justify-center text-muted text-sm">Không có dữ liệu trong khoảng đã chọn</div>}</div>
      {drill && <div className="card mt-2 p-0 overflow-auto"><div className="px-3 py-1.5 bg-surface-2 rounded-t-2xl flex justify-between text-sm"><b>Bản ghi gốc · {drill.t} ({drill.rows.length})</b><button onClick={() => setDrill(null)}>✕</button></div><RecordsTable rows={drill.rows} /></div>}
    </div>);
}
export function RecordsTable({ rows }: { rows: Record<string, unknown>[] }) {
  if (!rows.length) return <div className="p-3 text-muted text-sm">Không có bản ghi.</div>;
  ...
  return (
    <div className="space-y-2">
      <div className="card grid sm:grid-cols-2 lg:grid-cols-5 gap-2 items-end">
        <div className="lg:col-span-2"><label className="text-xs text-muted">Bảng dữ liệu ({tables.length})</label><select className="input" value={table} onChange={(e) => { setTable(e.target.value); setDim(""); setFilters({}); setFcol(""); }}>{tables.map((t) => <option key={t.table} value={t.table}>{t.label}</option>)}</select></div>
        <div><label className="text-xs text-muted">Trường số</label><select className="input" value={col} onChange={(e) => setCol(e.target.value)}>{nums.map((c) => <option key={c.name} value={c.name}>{c.name} ({c.type})</option>)}{jsonNums.length ? <optgroup label="Trường trong JSON">{jsonNums.map((k) => <option key={k} value={k}>{k}</option>)}</optgroup> : null}<option value="id">— đếm bản ghi —</option></select></div>
        <div><label className="text-xs text-muted">Cách gộp</label><select className="input" value={agg} onChange={(e) => setAgg(e.target.value as AnyChartProps["agg"])}><option value="sum">Tổng</option><option value="avg">Trung bình</option><option value="count">Đếm</option><option value="min">Nhỏ nhất</option><option value="max">Lớn nhất</option></select></div>
        <div><label className="text-xs text-muted">Nhóm theo (chồng cột / nhiều đường)</label><select className="input" value={dim} onChange={(e) => setDim(e.target.value)}><option value="">Tổng</option>{dims.map((d) => <option key={d} value={d}>{d}</option>)}</select></div>
        <div className="lg:col-span-3 flex flex-wrap gap-1 items-end"><div><label className="text-xs text-muted">Lọc theo cột</label><select className="input !w-44" value={fcol} onChange={(e) => setFcol(e.target.value)}><option value="">— chọn cột —</option>{dims.map((d) => <option key={d} value={d}>{d}</option>)}</select></div>
          {fcol && <div><label className="text-xs text-muted">Giá trị</label><select className="input !w-56" value={filters[fcol] ?? ""} onChange={(e) => setFilters({ ...filters, [fcol]: e.target.value })}><option value="">— tất cả —</option>{fvals.map((v) => <option key={v.v} value={v.v}>{v.v} ({v.n})</option>)}</select></div>}
        ...
        <div className="text-xs text-muted lg:col-span-5">Nguyên tắc: <b>bất kỳ trường số nào được nhập vào hệ thống đều có biểu đồ</b> — không cần lập trình thêm; thêm bảng/cột mới → danh sách này tự cập nhật. Bấm cột/điểm để xem bản ghi gốc & người ghi.</div>
      </div>
