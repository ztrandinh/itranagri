...
export function DeptGraph({ depts, io, processes, focus, onFocus }: { depts: R[]; io: R[]; processes: R[]; focus?: string | null; onFocus?: (code: string | null) => void }) {
  ...
  return (<div className="overflow-x-auto"><svg width={Math.max(width, 900)} height={height} style={{ fontFamily: "ui-sans-serif, system-ui" }}>
    ...
    {cols.map((b, ci) => <text key={b} x={20 + ci * 190} y={22} fontSize={11} fontWeight={700} fill={COLORS[b]}>{b.replace("_", " ")}</text>)}
  ...
  {hover && (() => { const e = edges.find((x) => `${x.from}>${x.to}` === hover); return e ? <div className="text-xs bg-white border rounded-lg p-2 mt-1 inline-block"><b>{dname(e.from)} → {dname(e.to)}</b>: {e.items.map((i, k) => <span key={k} className="ml-2">• {String(i.what)} <span className="text-slate-500">[{String(i.via)} · {String(i.process_code)}]</span></span>)}</div> : null; })()}
  </div>);
}
