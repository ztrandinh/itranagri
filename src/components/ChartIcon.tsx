...
export default function ChartIcon({ type, id, label }: { type: string; id: string; label?: string }) {
  ...
  return (<>
    ...
    {/* Trước đây là <div> tự chế (fixed inset-0), không Escape/focus-trap — dùng ở gần như mọi dòng
        bảng toàn app nên đây là điểm chặn phím phổ biến nhất. Nay dùng Sheet chuẩn có sẵn. */}
    <Sheet open={open} onClose={() => setOpen(false)} widthClassName="sm:max-w-3xl">
      <div className="flex items-center"><b>{cfg.label} · {label ?? id}</b><a className="ml-3 underline text-sm" href={`/xem/${type}/${encodeURIComponent(id)}`}>trang 360</a></div>
      <div className="grid md:grid-cols-2 gap-3 mt-3">{cfg.charts.map((ch) => <div key={ch.title} className="border rounded-xl p-2"><AnyChart title={ch.title} table={ch.table} col={ch.col} agg={ch.agg ?? "sum"} dim={ch.dim ?? null} filters={{ [ch.filter]: id, ...(ch.extra ?? {}) }} bucket="week" from={new Date(Date.now() - 90 * 86400e3).toISOString().slice(0, 10)} height={190} compare /></div>)}</div>
      <div className="text-xs text-muted mt-2">Đường xám nét đứt = kỳ trước; % thay đổi ở góc phải mỗi biểu đồ; bấm cột để xem bản ghi gốc.</div>
    </Sheet>
  </>);
}
