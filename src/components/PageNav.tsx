...
export default function PageNav({ dept, title }: { dept?: string | null; title?: string }) {
  ...
  return <div className="flex items-center gap-1 mb-2 text-sm">
    <button type="button" className="px-3 py-2.5 min-h-[40px] rounded-lg border bg-white hover:bg-slate-50 font-semibold" onClick={back} title="Quay lại trang trước">← Quay lại</button>
    <a className="px-3 py-2.5 min-h-[40px] rounded-lg border bg-white hover:bg-slate-50 font-semibold" href={home} title="Về phòng của tôi">🏠 Phòng tôi</a>
    ...
    <button type="button" className="px-3 py-2.5 min-h-[40px] rounded-lg border bg-white hover:bg-slate-50 font-semibold" onClick={() => window.history.forward()} title="Tiến tới trang vừa quay lại" aria-label="Tiến tới trang vừa quay lại">↷</button>
