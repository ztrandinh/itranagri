...
export default function Shell({ sess, title, children }: { sess: Sess; title?: string; children: React.ReactNode }) {
  ...
  return (
    <div className="min-h-screen flex bg-surface-2 text-ink">
      ...
      {side && <div className="fixed inset-0 z-40 md:hidden" onClick={() => setSide(false)}><div className="absolute inset-0 bg-black/40" /><aside ref={drawerRef} role="dialog" aria-modal="true" aria-label="Menu điều hướng" tabIndex={-1} className="absolute left-0 top-0 h-full w-72 bg-slate-900 text-slate-100 flex flex-col outline-none" onClick={(e) => e.stopPropagation()}><div className="flex items-center px-4 h-14 border-b border-slate-800 font-black text-emerald-400">ITRAN AGRI<button className="ml-auto text-muted" onClick={() => setSide(false)}>✕</button></div>{SideNav}</aside></div>}
