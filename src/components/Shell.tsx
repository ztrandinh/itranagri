...
import { IconHome, IconChevronDown, IconChevronRight, IconChevronsLeft, IconChevronsRight, IconX, IconMenu, IconHourglass, IconCheck, IconCloudOff } from "@/components/icons/UiIcons";
...
export default function Shell({ sess, title, children }: { sess: Sess; title?: string; children: React.ReactNode }) {
  ...
  const SideNav = (
    <nav className="flex-1 overflow-y-auto py-2 text-[15px]">
      <Link href="/trang-chu" className={`mx-2 mb-1 flex items-center gap-2 rounded-lg px-3 py-2 ${path === "/trang-chu" ? "bg-brand-tok text-white" : "text-slate-200 hover:bg-[var(--side-hover)]"}`}><IconHome size={18} />{!collapsed && <span>Trang chủ · khu vực</span>}</Link>
      ...
      {chiKhuCuaToi && !collapsed && (soKhuAn > 0 || xemHet) && (
        <button className="mx-2 mt-2 w-[calc(100%-1rem)] rounded-lg px-3 py-1.5 text-left text-xs text-slate-400 hover:bg-[var(--side-hover)] flex items-center gap-1.5"
          onClick={() => setXemHet(!xemHet)}>
          {xemHet ? <><IconChevronDown size={13} /> Thu gọn — chỉ hiện phòng tôi</> : <><IconChevronRight size={13} /> Xem thêm {soKhuAn} khu khác của trại</>}
        </button>)}
      ...
      <div className="flex-1 flex flex-col min-w-0">
        <header className="app-header sticky top-0 z-20 h-14 bg-white/90 backdrop-blur border-b border-line flex items-center gap-3 px-3">
          ...
          <div className="ml-auto flex items-center gap-2 text-sm">
            ...
            <Bell />
            <button onClick={() => setShowQ(!showQ)} className={`rounded-full px-2.5 py-1 text-xs font-semibold flex items-center gap-1 ${!online ? "bg-amber-400 text-black" : q.length ? "bg-amber-200 text-warning-tok" : "bg-brand-soft text-brand"}`} title="Hàng đợi đồng bộ">
              {online ? (q.length ? <><IconHourglass size={12} /> {q.length}</> : <><IconCheck size={12} /> đồng bộ</>) : <><IconCloudOff size={12} /> offline {q.length}</>}
            </button>
