...
export const ZONES: { key: string; icon: string; label: string; desc: string; roles: string[]; items: { href: string; label: string; roles?: string[] }[] }[] = [
  ...
  { key: "kho", icon: "🏬", label: "Kho · vật tư · mua hàng", desc: "9 kho, FEFO, ngày-tồn, kiểm kê, PO", roles: ["*"], items: [{ href: "/kho", label: "Kho" }, { href: "/khau-phan", label: "Khẩu phần tối ưu", roles: ["tech_head", "director", "owner"] }, { href: "/thiet-bi", label: "Thiết bị" }, { href: "/truy-xuat", label: "Truy xuất" }, { href: "/ban-do", label: "Bản đồ" }, { href: "/in-an", label: "In ấn" }] },
