...
export async function Page({ title, children }: { title?: string; children: (s: Sess) => React.ReactNode }) {
  ...
  const path = (await headers()).get("x-pathname") ?? ""; if (path && !canPage(path, sess.role)) return <Shell sess={sess} title={title}><div className="card"><b>Không có quyền xem trang này</b><div className="text-sm text-muted mt-1">Vai <b>{sess.role}</b> không thuộc phạm vi {path}. Liên hệ quản trị nếu cần cấp quyền.</div></div></Shell>;
