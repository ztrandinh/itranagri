...
export default function DanPanel({ sess }: { sess: Sess }) {
  ...
  const [rowsLoading, setRowsLoading] = useState(true); const [rowsError, setRowsError] = useState<string | null>(null);
  ...
  const load = async () => {
    ...
    try {
      const j = await fetch(`/api/list?${qs}`).then((r) => r.json());
      if (j.error) { setRowsError(String(j.error)); setRows([]); setTotal(0); } else { setRows(j.rows ?? []); setTotal(j.total ?? 0); }
    } catch { setRowsError("offline"); } finally { setRowsLoading(false); }
  };
    ...
    <div className="space-y-3">
      ...
      {tab === "tong" && (<div className="space-y-3">
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
          <div className="kpi"><div className="l">Tổng đàn cá thể (bò/dê)</div><div className="v">{fmt.n(totalHead)}</div><div className="text-xs">{(herd.rows ?? []).map((r) => `${r.species} ${r.status}: ${r.head_count}`).join(" · ")}</div></div>
          ...
          <button className="kpi text-left hover:bg-amber-50" onClick={() => goto({ attention: true })}><div className="l">Cần chú ý (bệnh · cách ly · ngưng thuốc · chờ tai · có việc)</div><div className="v text-warning-tok">{fmt.n((locs.rows ?? []).reduce((a, r) => a + Number(r.attention), 0))}</div><div className="text-xs underline">mở danh sách</div></button>
        ...
        <div className="card p-0 overflow-auto"><div className="px-3 py-2 font-bold bg-surface-2 rounded-t-2xl">Tầng 1 · Khu / chuồng / ô — bấm để xuống cá thể</div>
          <table className="tbl"><thead><tr><th className="pl-3">Khu/chuồng</th><th>Loại</th><th className="text-right">Cá thể</th><th className="text-right">Đàn nhóm</th><th className="text-right">Cần chú ý</th><th className="text-right">Việc mở</th></tr></thead><tbody>
            {(locs.rows ?? []).filter((r) => Number(r.head_individual) + Number(r.head_group) > 0 || Number(r.open_tasks) > 0).map((r) => <tr key={String(r.location_id)} className="hover:bg-green-50 cursor-pointer" role="button" tabIndex={0} aria-label={`Xem cá thể tại ${String(r.name)}`} onKeyDown={(e) => { if (e.key === "Enter" || e.key === " ") { e.preventDefault(); goto({ location: String(r.location_id) }); } }} onClick={() => goto({ location: String(r.location_id) })}><td className="pl-3 font-semibold">{String(r.name)}</td><td className="text-sm text-muted">{String(r.kind)}</td><td className="text-right">{fmt.n(r.head_individual)}</td><td className="text-right">{fmt.n(r.head_group)}</td><td className={`text-right ${Number(r.attention) ? "text-warning-tok font-bold" : ""}`}>{fmt.n(r.attention)}</td><td className="text-right">{fmt.n(r.open_tasks)}</td></tr>)}
          </tbody></table></div>
      </div>)}

      {tab === "cathe" && (<div className="space-y-2">
        <div className="card grid sm:grid-cols-6 gap-2 items-end">
          <div className="sm:col-span-2"><label className="text-xs text-muted">Tìm (số tai / mã / RFID / giống — không dấu)</label><input className="input" value={f.q} onChange={(e) => { setF({ ...f, q: e.target.value }); setPage(0); }} placeholder="B012, 00123, brahman…" /></div>
          <div><label className="text-xs text-muted">Chuồng</label><select className="input" value={f.location} onChange={(e) => { setF({ ...f, location: e.target.value }); setPage(0); }}><option value="">Tất cả</option>{(locations.rows ?? []).map((l) => <option key={String(l.id)} value={String(l.id)}>{String(l.name)}</option>)}</select></div>
          <div><label className="text-xs text-muted">Đàn</label><select className="input" value={f.group} onChange={(e) => { setF({ ...f, group: e.target.value }); setPage(0); }}><option value="">Tất cả</option>{(groups.rows ?? []).filter((g) => g.kind === "BO_NHOM" || g.kind === "DE").map((g) => <option key={String(g.id)} value={String(g.id)}>{String(g.name)}</option>)}</select></div>
          <div><label className="text-xs text-muted">Trạng thái</label><select className="input" value={f.status} onChange={(e) => { setF({ ...f, status: e.target.value }); setPage(0); }}><option value="">Đang nuôi</option>{["HAU_BI", "PHOI", "KHAM_THAI", "MANG_THAI", "DE", "NUOI_CON", "CAI_SUA", "CHO_PHOI", "SO_SINH", "THEO_ME", "PHA1", "PHA2", "PHA3", "BENH", "CACH_LY", "CHET", "XUAT"].map((x) => <option key={x}>{x}</option>)}</select></div>
        ...
        <div className="card p-0 overflow-auto"><DataBoundary loading={rowsLoading} error={rowsError} empty={!rows.length} onRetry={load} emptyText="Không có con nào khớp."><table className="tbl"><thead><tr><th></th><th className="pl-1">Số tai</th><th>Mã</th><th>Giống/giới</th><th>Trạng thái</th><th>Chuồng · Đàn</th><th className="text-right">Cân</th><th>Chú ý</th></tr></thead><tbody>
          {rows.map((a) => <tr key={a.id} className={a.attention ? "bg-warning-soft-tok" : ""}>
            ...
            <td className="font-bold"><Link className="underline py-2 -my-2" href={`/dan/${a.id}`}>{a.visual_tag ?? "—"}</Link></td><td className="font-mono text-xs">{a.id}</td><td className="text-sm">{a.breed} {a.sex}</td><td><span className={["BENH", "CACH_LY"].includes(a.status) ? "b-yel" : "b-gray"}>{a.status}</span></td>
            <td className="text-sm">{a.location_name}<br /><span className="text-muted">{a.group_name}</span></td><td className="text-right">{fmt.n(a.last_weight_kg)}</td>
      ...
      {tab === "dan" && (<div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-2">{(groups.rows ?? []).map((g) => <div key={String(g.id)} className="card"><div className="font-bold">{String(g.name)}</div><div className="text-sm text-muted">{String(g.id)} · {String(g.kind)} · {String(g.location_name ?? "")}</div><div className="text-2xl font-bold">{fmt.n(g.head_count)} <span className="text-sm font-normal">con</span></div><div className="text-sm">vào {fmt.d(g.started_at)} {g.all_in_all_out ? "· all-in-all-out" : ""} · chu kỳ {String(g.cycle_id ?? "—")}</div><div className="flex gap-3 text-sm mt-1"><Link className="underline py-2 -my-2" href={`/dan/${g.id}`}>Sự kiện đàn →</Link>{g.kind === "BO_NHOM" && <button className="underline py-2 -my-2" onClick={() => goto({ group: String(g.id) })}>Cá thể →</button>}</div></div>)}</div>)}
      ...
      {tab === "viec" && <div className="card p-0 overflow-auto"><div className="px-3 py-2 bg-surface-2 rounded-t-xl font-bold">Lịch sinh sản & hành động 30 ngày tới — tự suy từ sự kiện: khám thai (phối +60), chuẩn bị đẻ (phối +283 −14), tái phối (đẻ +60), cai sữa (180 ngày), cân 14 ngày, loại thải (không đậu ≥3 / &gt;8 tuổi), vỗ béo đủ ngày xuất, đang ngưng thuốc; việc ≤3 ngày tự sinh trong "Ca của tôi"</div><DataBoundary loading={acts.loading} error={acts.error} empty={!acts.rows?.length} onRetry={acts.reload} emptyText="Không có việc đàn trong 30 ngày."><table className="tbl text-sm"><thead><tr><th className="pl-3">Hạn</th><th>Việc</th><th>Con</th><th>Hạng</th><th>Ghi chú</th><th>Ưu tiên</th><th></th></tr></thead><tbody>{(acts.rows ?? []).map((r, i) => <tr key={i} className={new Date(String(r.due)) < new Date() ? "bg-danger-soft-tok" : r.priority === "CAO" ? "bg-warning-soft-tok" : ""}><td className="pl-3">{fmt.d(String(r.due))}</td><td className="font-semibold">{String(r.action)}</td><td><a className="underline py-2 -my-2" href={`/xem/animal/${r.animal_id}`}>{String(r.visual_tag ?? r.animal_id)}</a></td><td className="text-xs">{String(r.class_code)}</td><td className="text-xs">{String(r.note)}</td><td className="text-xs">{String(r.priority)}</td><td><ChartIcon type="animal" id={String(r.animal_id)} /></td></tr>)}</tbody></table></DataBoundary></div>}
      {tab === "chuky" && (<div className="card p-0 overflow-auto"><div className="px-3 py-2 font-bold bg-surface-2 rounded-t-2xl">Chu kỳ (vụ · lứa · đợt) — so sánh năm/lứa dựa vào đây; đóng chu kỳ = chốt số liệu</div><table className="tbl"><thead><tr><th className="pl-3">Mã</th><th>Loại</th><th>Đối tượng</th><th>Bắt đầu</th><th>Kết thúc</th><th>TT</th><th>Tổng kết</th><th></th></tr></thead><tbody>{(cycles.rows ?? []).map((c) => <tr key={String(c.id)}><td className="pl-3 font-mono text-xs">{String(c.id)}</td><td>{String(c.kind)}</td><td>{String(c.group_name ?? c.plot_name ?? "")}</td><td>{fmt.d(c.start_date)}</td><td>{fmt.d(c.end_date)}</td><td><span className={c.status === "MO" ? "b-grn" : "b-gray"}>{String(c.status)}</span></td><td className="text-xs">{c.summary ? JSON.stringify(c.summary).replace(/[{}"]/g, "") : ""}</td><td>{c.status === "MO" && ["team_lead", "tech_head", "director", "owner"].includes(sess.role) && <button className="btn-secondary !py-1 !px-2 !text-sm" onClick={async () => { if (await confirm({ title: "Đóng chu kỳ và chốt số liệu?", danger: true })) { await act("close_cycle", { id: c.id }); cycles.reload(); } }}>Đóng</button>}</td></tr>)}</tbody></table></div>)}
    </div>);
}
...
function BulkBar({ sel, groups, locations, onDone }: { sel: string[]; groups: Record<string, unknown>[]; locations: Record<string, unknown>[]; onDone: () => void }) {
  ...
  return <div className="card flex flex-wrap items-center gap-2 bg-warning-soft-tok"><b>{sel.length} con đã chọn</b>
...
function NewAnimal({ groups, lots, locations, onDone }: { groups: Record<string, unknown>[]; lots: Record<string, unknown>[]; locations: Record<string, unknown>[]; onDone: () => void }) {
  ...
  return (
    <div className="card space-y-2 bg-brand-soft">
      ...
      <button className="btn-primary" onClick={async () => { const r = await act("new_animal", { ...f, dam_id: f.dam_id || null, rfid: f.rfid || null, visual_tag: f.visual_tag || null, group_id: f.group_id || null, location_id: f.location_id || null, intake_lot_id: f.intake_lot_id || null }); if (r.code) toast.ok(`Đã tạo ${r.code}`); else toast.err(r.error); onDone(); }}>Tạo con mới</button>
    </div>);
}
