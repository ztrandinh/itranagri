...
export default function DanPanel({ sess }: { sess: Sess }) {
  ...
  return (
    <div className="space-y-3">
      ...
      {tab === "dan" && (<div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-2">{(groups.rows ?? []).map((g) => <div key={String(g.id)} className="card"><div className="font-bold">{String(g.name)}</div><div className="text-sm text-muted">{String(g.id)} · {String(g.kind)} · {String(g.location_name ?? "")}</div><div className="text-2xl font-bold">{fmt.n(g.head_count)} <span className="text-sm font-normal">con</span></div><div className="text-sm">vào {fmt.d(g.started_at)} {g.all_in_all_out ? "· all-in-all-out" : ""} · chu kỳ {String(g.cycle_id ?? "—")}</div><div className="flex gap-3 text-sm mt-1"><Link className="underline" href={`/dan/${g.id}`}>Sự kiện đàn →</Link>{g.kind === "BO_NHOM" && <button className="underline" onClick={() => goto({ group: String(g.id) })}>Cá thể →</button>}</div></div>)}</div>)}
