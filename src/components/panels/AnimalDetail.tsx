...
export default function AnimalDetail({ sess, code }: { sess: Sess; code: string }) {
  ...
  return (
    <div className="space-y-3">
      <div className="card">
        {an ? (<>
          <div className="flex flex-wrap items-center gap-2"><span className="text-3xl font-black">{String(an.visual_tag ?? "—")}</span><span className="font-mono">{String(an.id)}</span><span className="b-gray">{String(an.status)}</span>{an.tag_pending ? <span className="b-yel">chờ gắn tai</span> : null}{an.withdrawal_until && new Date(String(an.withdrawal_until)) > new Date() ? <span className="b-red">NGƯNG THUỐC đến {fmt.d(an.withdrawal_until)} — không xuất</span> : null}<span className="ml-auto text-xs text-stone-500">QR: /trace/{String(an.qr_token)}</span></div>
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 mt-2 text-sm">
            <div><div className="text-stone-500">Giống · giới</div><b>{String(an.breed ?? "")} {String(an.sex ?? "")}</b></div>
            <div><div className="text-stone-500">Ngày sinh</div><b>{fmt.d(an.birth_date)}</b> {an.birth_date ? <span>({Math.floor((Date.now() - new Date(String(an.birth_date)).getTime()) / 2.592e9)} tháng)</span> : null}</div>
            <div><div className="text-stone-500">Mẹ · cha/tinh</div><b>{String(an.dam_id ?? "—")}</b> · {String(an.sire_code ?? "—")}</div>
            <div><div className="text-stone-500">RFID</div><b className="font-mono">{String(an.rfid ?? "—")}</b></div>
            <div><div className="text-stone-500">Đàn</div><b>{String(an.group_name ?? "—")}</b></div>
            <div><div className="text-stone-500">Lô nhập</div><b>{String(an.intake_lot_id ?? "—")}</b> {an.intake_kind ? `(${String(an.intake_kind)} ${fmt.d(an.intake_date)})` : ""}</div>
            <div><div className="text-stone-500">Cân gần nhất</div><b>{fmt.n(an.last_weight_kg)} kg</b> {adg != null && <span>· ADG {adg} g/ngày</span>}</div>
            <div><div className="text-stone-500">Sở hữu · giá sổ đàn</div><b>{String(an.owner_type)}</b> · {fmt.vnd(an.unit_value)}</div>
          </div>
          ...
          {showTag && <div className="mt-2 flex gap-2 flex-wrap items-center bg-amber-50 p-2 rounded-xl"><select className="input !w-auto" value={tagType} onChange={(e) => setTagType(e.target.value)}><option>RFID</option><option>VISUAL</option><option>BOLUS</option></select><input className="input !w-64" placeholder="Giá trị thẻ" aria-label="Giá trị thẻ" value={tagVal} onChange={(e) => setTagVal(e.target.value)} /><button className="btn-primary !py-2" onClick={async () => { await act("assign_tag", { animal_id: an.id, tag_type: tagType, value: tagVal }); setTagVal(""); setShowTag(false); a.reload(); tags.reload(); }}>Lưu thẻ</button></div>}
          {tags.rows && tags.rows.length > 0 && <div className="text-xs text-stone-500 mt-1">Lịch sử thẻ: {tags.rows.map((t) => `${t.tag_type}:${t.value}${t.to_ts ? "(cũ)" : ""}`).join(" · ")}</div>}
