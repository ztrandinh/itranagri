    ...
    <div className="space-y-3">
      <div className="card">
        {an ? (<>
          ...
          {showTag && <div className="mt-2 flex gap-2 flex-wrap items-center bg-amber-50 p-2 rounded-xl"><select className="input !w-auto" value={tagType} onChange={(e) => setTagType(e.target.value)}><option>RFID</option><option>VISUAL</option><option>BOLUS</option></select><input className="input !w-64" placeholder="Giá trị thẻ" value={tagVal} onChange={(e) => setTagVal(e.target.value)} /><button className="btn-primary !py-2" onClick={async () => { await act("assign_tag", { animal_id: an.id, tag_type: tagType, value: tagVal }); setTagVal(""); setShowTag(false); a.reload(); tags.reload(); }}>Lưu thẻ</button></div>}
