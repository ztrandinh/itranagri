...
export function QcHoldPanel({ sess }: { sess: Sess }) {
  ...
  return <div className="space-y-3">
    ...
    <div className="card p-0 overflow-auto"><table className="tbl text-sm"><thead><tr><th className="pl-3">Lô/mẻ</th><th>Lý do</th><th>Mức</th><th>TT</th><th>Người giữ</th><th>Lúc</th><th></th></tr></thead><tbody>
      {(holds.rows ?? []).map((h) => <tr key={String(h.id)} className={h.status === "GIU" ? "bg-red-50" : ""}><td className="pl-3">{String(h.lot_id ?? h.batch_code)}</td><td className="text-xs">{String(h.reason)}</td><td className="text-xs">{String(h.severity)}</td><td className="text-xs">{h.status === "GIU" ? <span className="b-red">đang giữ</span> : <span className="b-gray">{String(h.status)}</span>}{h.disposition ? ` · ${String(h.disposition)}` : ""}</td><td className="text-xs">{String(h.held_by)}</td><td className="text-xs">{fmt.d(h.held_at)}</td><td className="whitespace-nowrap">{h.status === "GIU" && canRel && h.held_by !== sess.staffId && <><button className="btn-primary !py-0.5 !px-2 !text-xs mr-1" onClick={async () => { const d = prompt("Kết luận giải tỏa (tái kiểm đạt / lý do)"); if (d === null) return; await act("qc_release", { id: h.id, disposition: d, status: "GIAI_TOA" }); holds.reload(); lots.reload(); }}>Giải tỏa</button><button className="text-xs underline text-red-700" onClick={async () => { const d = prompt("Xử lý (hủy/hạ cấp) — lô sẽ chuyển THU_HOI"); if (!d) return; await act("qc_release", { id: h.id, disposition: d, status: "HUY_BO" }); holds.reload(); lots.reload(); }}>hủy/thu hồi</button></>}</td></tr>)}</tbody></table></div>
  </div>;
}
