...
export function SupplierReturnPanel({ sess }: { sess: Sess }) {
  ...
  return <div className="space-y-3">
    ...
    {f && <div className="card grid sm:grid-cols-4 gap-2"><select className="input" value={String(f.supplier_id)} onChange={(e) => setF({ ...f, supplier_id: e.target.value })}><option value="">— NCC —</option>{(partners.rows ?? []).filter((p) => String(p.kind ?? "").includes("NCC") || String(p.role ?? "").includes("SUPP") || true).slice(0, 200).map((p) => <option key={String(p.id)} value={String(p.id)}>{String(p.name)}</option>)}</select><select className="input" value={String(f.sku)} onChange={(e) => setF({ ...f, sku: e.target.value })}><option value="">— hàng —</option>{(products.rows ?? []).map((p) => <option key={String(p.sku)} value={String(p.sku)}>{String(p.name)}</option>)}</select><input className="input" type="number" placeholder="SL" aria-label="SL" value={String(f.qty)} onChange={(e) => setF({ ...f, qty: e.target.value })} /><input className="input" type="number" placeholder="Đơn giá" aria-label="Đơn giá" value={String(f.unit_cost)} onChange={(e) => { const uc = e.target.value; setF({ ...f, unit_cost: uc, amount: String(Number(uc) * Number(f.qty)) }); }} /><input className="input sm:col-span-3" placeholder="Lý do (hàng lỗi/sai/kém chất lượng)" aria-label="Lý do (hàng lỗi/sai/kém chất lượng)" value={String(f.reason)} onChange={(e) => setF({ ...f, reason: e.target.value })} /><input className="input" type="number" placeholder="Thành tiền" aria-label="Thành tiền" value={String(f.amount)} onChange={(e) => setF({ ...f, amount: e.target.value })} /><div className="sm:col-span-4 flex gap-2"><button className="btn-primary !py-1" disabled={!f.sku || !f.qty} onClick={async () => { const j = await act("supplier_return", f); setMsg(j.error ? String(j.error) : "Đã tạo phiếu trả " + j.id); setF(null); rets.reload(); }}>Tạo phiếu</button><button className="btn-secondary !py-1" onClick={() => setF(null)}>Hủy</button></div></div>}
    {msg && <div className="text-sm text-emerald-800">{msg}</div>}
