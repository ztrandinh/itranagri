  ...
  return (
    <div className="space-y-3">
      ...
      {tab === "nhap" && (<div className="space-y-3">
        {!cur && <DataBoundary loading={pending.loading} error={pending.error} empty={!pending.rows?.length} onRetry={pending.reload} emptyText="Không có phiếu chờ số hóa."><div className="grid sm:grid-cols-2 gap-2">{(pending.rows ?? []).map((p) => <button key={String(p.id)} className="tile" onClick={() => { setCur(p); setN(0); }}><span className="font-bold">{String(p.serial)}</span><span className="text-sm text-muted">{String(p.form_code)} · chụp {fmt.dt(p.ts)} {new Date(String(p.ts)) < new Date(Date.now() - 86400e3) && <span className="b-yel">&gt;24h</span>}</span>{p.anomaly ? <span className="text-danger-tok text-sm">⚠ {String(p.anomaly)}</span> : null}</button>)}</div></DataBoundary>}
        {cur && forms && (<div className="grid lg:grid-cols-2 gap-3">
          <div className="card"><div className="flex justify-between"><b>{String(cur.serial)}</b><button className="underline text-sm py-2 -my-2" onClick={() => setCur(null)}>← danh sách</button></div>{cur.photo_url ? <img src={String(cur.photo_url)} alt="phiếu" loading="lazy" className="mt-2 w-full rounded-xl border" /> : <div className="text-muted mt-2">Không có ảnh — đối chiếu bản cứng.</div>}<div className="mt-2 text-sm">Đã nhập {n} dòng từ tờ này.</div><button className="btn-primary mt-2 w-full" onClick={async () => { await act("digitize_paper", { id: cur.id }); setCur(null); pending.reload(); all.reload(); }}>✓ ĐÃ SỐ HÓA xong tờ này</button></div>
      ...
      {tab === "audit" && (<div className="card p-0 overflow-auto"><DataBoundary loading={all.loading} error={all.error} empty={!all.rows?.length} onRetry={all.reload} emptyText="Chưa có phiếu giấy."><table className="tbl"><thead><tr><th className="pl-3">Seri</th><th>Mẫu</th><th>Chụp</th><th>Số hóa</th><th>Người nhập</th><th>Bất thường</th><th>Ảnh</th></tr></thead><tbody>{(all.rows ?? []).map((p) => <tr key={String(p.id)}><td className="pl-3 font-mono">{String(p.serial)}</td><td>{String(p.form_code)}</td><td>{fmt.dt(p.ts)}</td><td>{p.digitized ? <span className="b-grn">{fmt.dt(p.digitized_ts)}</span> : <span className="b-yel">chưa</span>}</td><td>{String(p.digitized_by ?? "")}</td><td className="text-danger-tok">{String(p.anomaly ?? "")}</td><td>{p.photo_url ? <a href={String(p.photo_url)} target="_blank">📷</a> : null}</td></tr>)}</tbody></table></DataBoundary></div>)}
    </div>);
}
