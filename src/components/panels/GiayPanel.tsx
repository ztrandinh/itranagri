    ...
    <div className="space-y-3">
      ...
      {tab === "nhap" && (<div className="space-y-3">
        ...
        {cur && forms && (<div className="grid lg:grid-cols-2 gap-3">
          <div className="card"><div className="flex justify-between"><b>{String(cur.serial)}</b><button className="underline text-sm py-2 -my-2" onClick={() => setCur(null)}>← danh sách</button></div>{cur.photo_url ? <img src={String(cur.photo_url)} alt="phiếu" className="mt-2 w-full rounded-xl border" /> : <div className="text-muted mt-2">Không có ảnh — đối chiếu bản cứng.</div>}<div className="mt-2 text-sm">Đã nhập {n} dòng từ tờ này.</div><button className="btn-primary mt-2 w-full" onClick={async () => { await act("digitize_paper", { id: cur.id }); setCur(null); pending.reload(); all.reload(); }}>✓ ĐÃ SỐ HÓA xong tờ này</button></div>
