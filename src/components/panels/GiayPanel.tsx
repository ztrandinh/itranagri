...
export default function GiayPanel({ sess }: { sess: Sess }) {
  ...
  return (
    <div className="space-y-3">
      ...
      {tab === "audit" && (<div className="card p-0 overflow-auto"><table className="tbl"><thead><tr><th className="pl-3">Seri</th><th>Mẫu</th><th>Chụp</th><th>Số hóa</th><th>Người nhập</th><th>Bất thường</th><th>Ảnh</th></tr></thead><tbody>{(all.rows ?? []).map((p) => <tr key={String(p.id)}><td className="pl-3 font-mono">{String(p.serial)}</td><td>{String(p.form_code)}</td><td>{fmt.dt(p.ts)}</td><td>{p.digitized ? <span className="b-grn">{fmt.dt(p.digitized_ts)}</span> : <span className="b-yel">chưa</span>}</td><td>{String(p.digitized_by ?? "")}</td><td className="text-danger-tok">{String(p.anomaly ?? "")}</td><td>{p.photo_url ? <a href={String(p.photo_url)} target="_blank">📷</a> : null}</td></tr>)}</tbody></table></div>)}
    </div>);
}
