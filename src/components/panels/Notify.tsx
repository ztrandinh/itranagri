...
export function Bell() {
  ...
  return (<div className="relative">
    <button className={`rounded-full px-2.5 py-1 text-xs font-semibold min-h-[44px] ${n ? "bg-red-500 text-white" : "bg-surface-2 text-ink"}`} onClick={() => { setOpen(!open); void load(); }} title="Thông báo">🔔 {n ? (n > 99 ? "99+" : n) : ""}</button>
    {open && <div className="absolute right-0 mt-1 w-[92vw] sm:w-[420px] max-h-[70vh] overflow-auto bg-surface text-ink rounded-2xl shadow-xl border z-50 p-2">
      <div className="flex items-center justify-between px-1 pb-1 border-b"><b>Hộp thư của tôi</b><div className="flex gap-2 text-xs"><button className="underline py-2 -my-2" onClick={async () => { await fetch("/api/notifications", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ all: true }) }); load(); }}>đã đọc hết</button><button className="underline py-2 -my-2" title="Nhận thông báo nổi trên điện thoại kể cả khi đóng app" onClick={async () => { try { const perm = await Notification.requestPermission(); if (perm !== "granted") return toast.err("Bạn chưa cho phép thông báo"); const reg = await navigator.serviceWorker.ready; const { key } = await fetch("/api/push").then((r) => r.json()); if (!key) return toast.err("Máy chủ chưa cấu hình VAPID"); const sub = await reg.pushManager.subscribe({ userVisibleOnly: true, applicationServerKey: key }); await fetch("/api/push", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ subscription: sub }) }); toast.ok("Đã bật thông báo nổi trên thiết bị này"); } catch (e) { toast.err("Không bật được: " + (e as Error).message); } }}>🔔 bật push</button><a className="underline py-2 -my-2" href="/canh-bao?tab=caidat">cài đặt</a></div></div>
      {rows.map((r) => <a key={r.id} href={r.link ?? "#"} className={`block px-2 py-2 border-b hover:bg-stone-50 ${r.read_at ? "opacity-60" : ""}`} onClick={async () => { await fetch("/api/notifications", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ ids: [r.id] }) }); }}><div className="flex items-center gap-2"><span className={LV(r.level)}>{r.level}</span><span className="font-semibold text-sm flex-1">{r.title}</span><span className="text-xs text-muted">{fmt.dt(r.ts)}</span></div>{r.body && <div className="text-sm text-muted">{r.body.slice(0, 140)}</div>}</a>)}
      {!rows.length && <div className="p-3 text-muted text-sm">Chưa có thông báo.</div>}
    </div>}
  </div>);
}
  ...
  return (
    <div className="space-y-4">
      <div className="card"><h3 className="font-bold">Sở thích nhận thông báo của tôi ({sess.staffName})</h3>
        ...
        <div className="text-xs text-muted mt-1">Zalo/SMS/email: kênh được xếp hàng (cột sent) — kích hoạt khi có Zalo OA/brandname; app luôn có.</div></div>
      {canW && <div className="card"><h3 className="font-bold">Tạo / sửa luật cảnh báo (không cần code — thêm luật mới bất kỳ lúc nào)</h3>
        ...
        <div className="flex gap-2 mt-2"><button className="btn-primary !py-2" onClick={async () => { if (!f.code || !f.name) return toast.err("Thiếu mã/tên"); const expr: Record<string, unknown> = { type: t, op: f.op, value: f.value }; if (t === "stock_days") expr.sku = f.sku; if (t === "metric_threshold") expr.metric = f.metric; if (t === "missing_event") { expr.table = f.table ?? "feed_logs"; expr.after_hour = f.value; } const j = await act("save_alert_rule", { ...f, expr }); setMsg(j.ok ? `Đã lưu ${f.code} v${j.version}` : j.error); rules.reload(); }}>Lưu luật</button><button className="btn-secondary !py-2" onClick={async () => { const j = await act("run_rules_now", {}); setMsg(`Đã chạy: ${(j.fired ?? []).length} cảnh báo, ${j.notified} thông báo`); rules.reload(); }}>▶ Chạy luật ngay</button>{msg && <span className="text-sm self-center">{msg}</span>}</div></div>}
      <div className="card p-0 overflow-auto"><div className="px-3 py-2 font-bold bg-surface-2 rounded-t-2xl">Danh mục luật (có phiên bản, có vết ai sửa)</div><DataBoundary loading={rules.loading} error={rules.error} empty={!rules.rows?.length} onRetry={rules.reload} emptyText="Chưa có luật cảnh báo."><table className="tbl"><thead><tr><th className="pl-3">Mã</th><th>Tên</th><th>Loại</th><th>Mức</th><th>Người nhận</th><th>Kênh</th><th>Phạm vi</th><th className="text-right">Đã nổ</th><th>TT</th></tr></thead><tbody>
        {(rules.rows ?? []).map((r) => <tr key={`${r.code}-${r.version}-${r.farm_id}`} className={r.active ? "" : "opacity-50"}><td className="pl-3 font-mono text-sm">{String(r.code)} <span className="text-xs">v{String(r.version)}</span></td><td>{String(r.name)}</td><td className="text-xs">{String((r.expr as Record<string, unknown>)?.type ?? r.source)} {JSON.stringify(r.expr).slice(0, 50)}</td><td><span className={LV(String(r.level))}>{String(r.level)}</span></td><td className="text-xs">{(r.recipients as string[] ?? []).join(", ")}</td><td className="text-xs">{(r.channels as string[] ?? []).join(",")}</td><td className="text-xs">{String(r.farm_id)}</td><td className="text-right">{String(r.fire_count ?? 0)}</td><td>{canW && <button className="underline text-xs py-2 -my-2" onClick={async () => { await act("toggle_alert_rule", { code: r.code, version: r.version, active: !r.active }); rules.reload(); }}>{r.active ? "tắt" : "bật"}</button>}</td></tr>)}
