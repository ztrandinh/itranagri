...
export function Bell() {
  ...
  return (<div className="relative">
    <button className={`rounded-full px-2.5 py-1 text-xs font-semibold ${n ? "bg-red-500 text-white" : "bg-surface-2 text-ink"}`} onClick={() => { setOpen(!open); void load(); }} title="Thông báo">🔔 {n ? (n > 99 ? "99+" : n) : ""}</button>
    {open && <div className="absolute right-0 mt-1 w-[92vw] sm:w-[420px] max-h-[70vh] overflow-auto bg-surface text-ink rounded-2xl shadow-xl border z-50 p-2">
      <div className="flex items-center justify-between px-1 pb-1 border-b"><b>Hộp thư của tôi</b><div className="flex gap-2 text-xs"><button className="underline" onClick={async () => { await fetch("/api/notifications", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ all: true }) }); load(); }}>đã đọc hết</button><button className="underline" title="Nhận thông báo nổi trên điện thoại kể cả khi đóng app" onClick={async () => { try { const perm = await Notification.requestPermission(); if (perm !== "granted") return toast.err("Bạn chưa cho phép thông báo"); const reg = await navigator.serviceWorker.ready; const { key } = await fetch("/api/push").then((r) => r.json()); if (!key) return toast.err("Máy chủ chưa cấu hình VAPID"); const sub = await reg.pushManager.subscribe({ userVisibleOnly: true, applicationServerKey: key }); await fetch("/api/push", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ subscription: sub }) }); toast.ok("Đã bật thông báo nổi trên thiết bị này"); } catch (e) { toast.err("Không bật được: " + (e as Error).message); } }}>🔔 bật push</button><a className="underline" href="/canh-bao?tab=caidat">cài đặt</a></div></div>
