...
export function MeterReadingPanel({ sess }: { sess: Sess }) {
  ...
  return <div className="space-y-3">
    ...
    <div className="card p-0 overflow-auto"><DataBoundary loading={latest.loading} error={latest.error} empty={!rows.length} onRetry={latest.reload} emptyText='Chưa có số đọc — bấm "＋ Ghi số đọc". Cấu hình chỉ số ở Quản trị DL › reading_metrics.'><table className="tbl text-sm"><thead><tr><th className="pl-3">Máy</th><th>Chỉ số</th><th className="text-right">Số đọc</th><th className="text-right">Chênh kỳ</th><th>Seri giấy</th><th>Lúc</th><th>Người</th><th></th></tr></thead><tbody>
...
export function RecordingCompliancePanel({ sess }: { sess: Sess }) {
  ...
  return <div className="space-y-3">
    ...
    <div className="card p-0 overflow-auto"><DataBoundary loading={due.loading} error={due.error} empty={!rows.length} onRetry={due.reload} emptyText="Chưa khai báo khâu bắt buộc — thêm ở Quản trị DL › recording_obligations."><table className="tbl text-sm"><thead><tr><th className="pl-3">Khâu ghi chép</th><th>Phòng</th><th>Tần suất</th><th>Ghi cuối</th><th>Trạng thái</th><th className="text-right">Trễ (h)</th><th>Phụ trách</th></tr></thead><tbody>
