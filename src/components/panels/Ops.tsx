...
export function CanhBaoPanel({ sess }: { sess: Sess }) {
  ...
  return (
    <div className="space-y-3">
      <div className="flex gap-2 items-center">{[["group", "Theo luật (30 ngày)"], ["open", `Chưa xử lý (${open.rows?.length ?? 0})`], ["all", "Tất cả"], ["caidat", "⚙ Cài đặt cảnh báo · Hộp thư"]].map(([k, l]) => <button key={k} className={`px-4 py-2 rounded-xl font-semibold ${tab === k ? "bg-green-700 text-white" : "bg-white border"}`} onClick={() => setTab(k as typeof tab)}>{l}</button>)}
  ...
  return (
    <div className="space-y-3">
      ...
      {tab === "don" && <div className="space-y-3">
        ...
        <div className="text-xs text-slate-500">Luồng: Nhập → <b>Giữ hàng</b> (FEFO theo lô; thiếu → tự sinh lệnh SX cho D5) → Chốt → <b>Phiếu soạn</b> (in cho kho) → <b>Xuất kho</b> (từng lô, → GIAO) → Hoàn tất. Đơn từ sàn/portal cũng vào đây.</div></div>}
      ...
      {tab === "nn" && <div className="space-y-3">
        ...
        <div className="text-xs text-stone-500">Đợt sau: cổng khách hàng (ảnh mốc, camera theo khung giờ, chat SLA), báo cáo tháng tự sinh cho từng khách.</div></div>}
      ...
      {tab === "ban" && <div className="grid md:grid-cols-2 gap-3">
        ...
        <div className="card"><h3 className="font-bold">Công nợ</h3><table className="tbl"><tbody>{(recv.rows ?? []).map((r) => <tr key={String(r.id)}><td>{String(r.name)}</td><td className="text-right">{fmt.vnd(r.unpaid)}</td><td className={Number(r.days) > 30 ? "text-red-700 font-bold" : Number(r.days) > 15 ? "text-amber-700" : ""}>{String(r.days)} ngày {Number(r.days) > 30 && "· NGỪNG GIAO"}</td></tr>)}</tbody></table>
          <h3 className="font-bold mt-3">Tuổi nợ</h3><table className="tbl"><tbody>{(aging.rows ?? []).map((r) => <tr key={String(r.partner_id)}><td>{String(r.name)}</td><td className="text-right">{fmt.vnd(r.d0_15 ?? 0)}</td><td className="text-right">{fmt.vnd(r.d16_30 ?? 0)}</td><td className={`text-right ${Number(r.d30p) > 0 ? "text-red-700 font-bold" : ""}`}>{fmt.vnd(r.d30p ?? 0)}</td></tr>)}</tbody></table>
  ...
  return (
    <div className="space-y-3">
      ...
      {q && (<><div className="grid md:grid-cols-3 gap-3">
        ...
        <div className="card"><h3 className="font-bold">← 1 bước lùi (nguyên liệu)</h3><DataBoundary loading={tree.loading} error={tree.error} empty={!back.length} onRetry={tree.reload} emptyText="Không có mẻ dùng lô này làm đầu ra."><ul className="text-sm">{back.map((r, i) => <li key={i}>{"—".repeat(Number(r.depth))} {String(r.input_lot)} <span className="text-stone-500">(mẻ {String(r.batch_code)})</span></li>)}</ul></DataBoundary></div>
