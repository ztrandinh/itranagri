  ...
  return (
    <div className="space-y-3">
      ...
      <div className="card p-0 overflow-auto"><DataBoundary loading={latest.loading} error={latest.error} empty={!latest.rows?.length} onRetry={latest.reload} emptyText={'Chưa có kết quả — bấm "Chạy đối soát".'}><table className="tbl"><thead><tr><th className="pl-3">Mã</th><th>Đối soát</th><th>Kỳ</th><th className="text-right">Vế A</th><th className="text-right">Vế B</th><th className="text-right">Lệch</th><th>Trạng thái</th><th></th></tr></thead><tbody>
        {(latest.rows ?? []).map((r) => <tr key={String(r.id)} className={r.status === "DO" ? "bg-danger-soft-tok" : r.status === "VANG" ? "bg-warning-soft-tok" : ""}><td className="pl-3 font-bold"><button className="underline" onClick={() => setRule(String(r.rule_code))}>{String(r.rule_code)}</button></td><td>{String(r.name)}</td><td>{fmt.d(r.period)}</td><td className="text-right">{fmt.n(r.expected, 1)}</td><td className="text-right">{fmt.n(r.actual, 1)}</td><td className="text-right font-bold">{r.diff_pct != null ? fmt.n(r.diff_pct, 2) : "—"}</td><td><span className={LV(String(r.status))}>{String(r.status)}</span>{(r.detail as Record<string, unknown>)?.err ? <div className="text-xs text-danger-tok">{String((r.detail as Record<string, unknown>).err).slice(0, 80)}</div> : null}</td><td>{["DO","VANG"].includes(String(r.status)) && !r.acked_by && ["tech_head","director","owner"].includes(sess.role) && <button className="btn-secondary !py-1 !px-2 !text-sm" onClick={async () => { await act("ack_recon", { id: r.id }); latest.reload(); }}>Đã xem</button>}{r.acked_by ? <span className="text-xs">✓ {String(r.acked_by)}</span> : null}</td></tr>)}
...
export function CanhBaoPanel({ sess }: { sess: Sess }) {
  ...
  return (
    <div className="space-y-3">
      <div className="flex gap-2 items-center">{[["group", "Theo luật (30 ngày)"], ["open", `Chưa xử lý (${open.rows?.length ?? 0})`], ["all", "Tất cả"], ["caidat", "⚙ Cài đặt cảnh báo · Hộp thư"]].map(([k, l]) => <button key={k} className={`px-4 py-2 rounded-xl font-semibold ${tab === k ? "bg-brand-tok text-white" : "bg-surface border"}`} onClick={() => setTab(k as typeof tab)}>{l}</button>)}
