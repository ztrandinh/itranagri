...
export function CanhBaoPanel({ sess }: { sess: Sess }) {
  const open = useData("alerts_open"); const all = useData("alerts_all"); const [tab, setTab] = useState<"open" | "all">("open");
  const rows = tab === "open" ? open.rows : all.rows;
  return (
    <div className="space-y-3">
      <div className="flex gap-2 items-center">{[["open", `Chưa xử lý (${open.rows?.length ?? 0})`], ["all", "Tất cả"]].map(([k, l]) => <button key={k} className={`px-4 py-2 rounded-xl font-semibold ${tab === k ? "bg-green-700 text-white" : "bg-white border"}`} onClick={() => setTab(k as typeof tab)}>{l}</button>)}
