...
export function CanhBaoPanel({ sess }: { sess: Sess }) {
  ...
  return (
    <div className="space-y-3">
      <div className="flex gap-2 items-center">{[["group", "Theo luật (30 ngày)"], ["open", `Chưa xử lý (${open.rows?.length ?? 0})`], ["all", "Tất cả"], ["caidat", "⚙ Cài đặt cảnh báo · Hộp thư"]].map(([k, l]) => <button key={k} className={`px-4 py-2 rounded-xl font-semibold min-h-[40px] ${tab === k ? "bg-brand-tok text-white" : "bg-surface border"}`} onClick={() => setTab(k as typeof tab)}>{l}</button>)}
