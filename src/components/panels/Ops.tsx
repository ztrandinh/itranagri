...
export function DoiSoatPanel({ sess }: { sess: Sess }) {
  ...
  return (
    <div className="space-y-3">
      ...
      <div className="card p-0 overflow-auto"><DataBoundary loading={latest.loading} error={latest.error} empty={!latest.rows?.length} onRetry={latest.reload} emptyText={'Chưa có kết quả — bấm "Chạy đối soát".'}><table className="tbl"><thead><tr><th className="pl-3">Mã</th><th>Đối soát</th><th>Kỳ</th><th className="text-right">Vế A</th><th className="text-right">Vế B</th><th className="text-right">Lệch</th><th>Trạng thái</th><th></th></tr></thead><tbody>
        {(latest.rows ?? []).map((r) => <tr key={String(r.id)} className={r.status === "DO" ? "bg-danger-soft-tok" : r.status === "VANG" ? "bg-warning-soft-tok" : ""}><td className="pl-3 font-bold"><button className="underline py-2 -my-2" onClick={() => setRule(String(r.rule_code))}>{String(r.rule_code)}</button></td><td>{String(r.name)}</td><td>{fmt.d(r.period)}</td><td className="text-right">{fmt.n(r.expected, 1)}</td><td className="text-right">{fmt.n(r.actual, 1)}</td><td className="text-right font-bold">{r.diff_pct != null ? fmt.n(r.diff_pct, 2) : "—"}</td><td><span className={LV(String(r.status))}>{String(r.status)}</span>{(r.detail as Record<string, unknown>)?.err ? <div className="text-xs text-danger-tok">{String((r.detail as Record<string, unknown>).err).slice(0, 80)}</div> : null}</td><td>{["DO","VANG"].includes(String(r.status)) && !r.acked_by && ["tech_head","director","owner"].includes(sess.role) && <button className="btn-secondary !py-2 !px-2 !text-sm" onClick={async () => { await act("ack_recon", { id: r.id }); latest.reload(); }}>Đã xem</button>}{r.acked_by ? <span className="text-xs">✓ {String(r.acked_by)}</span> : null}</td></tr>)}
        </tbody></table></DataBoundary></div>
...
export function CanhBaoPanel({ sess }: { sess: Sess }) {
  ...
  const src = tab === "open" ? open : all;
  const rows = src.rows;
  return (
    <div className="space-y-3">
      <Tabs items={[["group", "Theo luật (30 ngày)"], ["open", `Chưa xử lý (${open.rows?.length ?? 0})`], ["all", "Tất cả"], ["caidat", "⚙ Cài đặt cảnh báo · Hộp thư"]]} value={tab} onChange={(k) => setTab(k as typeof tab)} right={["tech_head","director","owner","it_engineer"].includes(sess.role) ? <button className="btn-secondary !py-2" onClick={async () => { await fetch(`/api/jobs/alerts?farm=${sess.farmId}`, { method: "POST" }); open.reload(); all.reload(); }}>▶ Quét cảnh báo ngay</button> : undefined} />
      {tab === "caidat" && <AlertSettings sess={sess} />}
      {tab === "group" && <div className="card p-0 overflow-auto"><table className="tbl"><thead><tr><th className="pl-3">Luật</th><th>Mức</th><th className="text-right">Số lần</th><th className="text-right">Chưa xử lý</th><th>Lần cuối</th><th>Nội dung gần nhất</th></tr></thead><tbody>{(grouped.rows ?? []).map((g, i) => <tr key={i} className={Number(g.open) ? (g.level === "DO" ? "bg-danger-soft-tok" : "bg-warning-soft-tok") : ""}><td className="pl-3 font-bold">{String(g.rule_code)}</td><td><span className={LV(String(g.level))}>{String(g.level)}</span></td><td className="text-right">{String(g.n)}</td><td className="text-right font-bold">{String(g.open)}</td><td className="text-sm">{fmt.dt(g.last_ts)}</td><td className="text-sm">{String(g.last_subject ?? "").slice(0, 90)}</td></tr>)}</tbody></table><div className="text-xs text-muted px-3 py-2">Ở quy mô lớn xem theo luật trước; mở tab "Chưa xử lý" để ack từng cái. Cảnh báo lặp mỗi đêm (RC) không tràn màn.</div></div>}
      {tab !== "group" && <DataBoundary loading={src.loading} error={src.error} empty={!rows?.length} onRetry={src.reload} emptyText="Không có cảnh báo."><div className="space-y-2">{(rows ?? []).map((a) => <div key={String(a.id)} className={`card flex items-start gap-3 ${a.level === "DO" ? "border-danger-tok" : a.level === "VANG" ? "border-amber-300" : ""}`}><span className={LV(String(a.level))}>{String(a.level)}</span><div className="flex-1"><div className="font-semibold">{String(a.rule_code)} · {String(a.subject)}</div><div className="text-xs text-muted">{fmt.dt(a.ts)} {a.acked_by ? `· đã nhận ${String(a.acked_by)} ${fmt.dt(a.acked_at)}` : ""}</div></div>{!a.acked_by && <button className="btn-primary !py-2 !px-3 !text-sm" onClick={async () => { await act("ack_alert", { id: a.id }); open.reload(); all.reload(); }}>Đã nhận</button>}</div>)}</div></DataBoundary>}
    </div>);
}
...
export function SopPanel({ code }: { code?: string }) {
  ...
  if (code) { const v = one.rows?.[0]; if (!v) return <div className="card">Đang tải…</div>;
    return (<div className="card space-y-2"><Link href="/sop" className="underline text-sm">← Thư viện</Link><h2 className="text-2xl font-bold">{String(v.code)} · {String(v.title)} <span className="b-gray">v{String(v.version)} {String(v.status)}</span></h2>
      ...
      {v.video_url ? <a className="underline py-2 -my-2" href={String(v.video_url)}>▶ Video ≤5'</a> : null}</div>); }
  return (<div className="card p-0 overflow-auto"><div className="px-3 py-2 font-bold bg-surface-2 rounded-t-2xl">Thư viện SOP (chuẩn 10+1 trường) — không SOP ký = không giao việc</div><table className="tbl"><thead><tr><th className="pl-3">Mã</th><th>Tên</th><th>Bộ phận</th><th>Chuỗi/L2</th><th>Phiên bản</th><th>Rà hạn</th><th>Trạng thái</th></tr></thead><tbody>{(sops.rows ?? []).map((s) => <tr key={String(s.code)}><td className="pl-3 font-mono"><Link className="underline" href={`/sop/${s.code}`}>{String(s.code)}</Link></td><td>{String(s.title)}</td><td>{String(s.dept)}</td><td className="text-sm">{String(s.l2_group ?? "")}</td><td>{s.version ? `v${s.version}` : "—"}</td><td className={s.review_due && new Date(String(s.review_due)) < new Date(Date.now() + 30 * 86400e3) ? "text-warning-tok" : ""}>{fmt.d(s.review_due)}</td><td><span className={s.status === "BAN_HANH" ? "b-grn" : "b-yel"}>{String(s.status)}</span></td></tr>)}</tbody></table></div>);
}
...
function LivestockSalePanel({ sess }: { sess: Sess }) {
  ...
  if (!can) return <div className="card text-sm text-muted">Chỉ tổ trưởng / kỹ thuật trưởng / GĐ được lập phiếu xuất bán vật hơi.</div>;
  return (
    <div className="space-y-3">
      <div className="card text-sm"><b>Xuất bán vật hơi — chỉ bán con sống, truy xuất từng cá thể ⇄ đơn</b>
        <div className="text-xs text-muted mt-1"><b>Vì sao:</b> xuất khẩu bắt buộc truy được con nào bán cho ai (EPCIS 2.0). Chọn con → hệ thống tự sinh sự kiện XUẤT từng con, giảm đầu đàn, nối đơn⇄con; <b>tự chặn con còn ngưng thuốc / chưa có cân</b>. Chụp ảnh phiếu cân giấy đính kèm = 2 bộ hồ sơ (giấy + số), lưu BM06.</div></div>
      ...
      <div className="card p-0 overflow-auto max-h-[420px]"><table className="tbl text-sm"><thead><tr><th className="pl-3"></th><th>Mã con</th><th>Giống</th><th className="text-right">Cân (kg)</th><th>Trạng thái</th></tr></thead><tbody>
        {list.map((a) => { const bl = blockOf(a); return <tr key={String(a.id)} className={bl ? "opacity-40" : sel[String(a.id)] ? "bg-brand-soft" : ""}>
        ...
        {!list.length && <tr><td colSpan={5} className="p-3 text-muted">Không có {sp} nào trong đàn.</td></tr>}
      </tbody></table></div>
      ...
      {done && <div className="card bg-brand-soft text-sm"><b>Đã lập đơn {done.id}</b> — {done.n} con chuyển XUẤT, đầu đàn tự giảm, đơn⇄con đã nối. Xem <Link className="underline" href="/truy-xuat">Truy xuất</Link>{photo ? " · ảnh phiếu đã lưu BM06" : ""}.</div>}
    </div>);
}
...
export function BanHangPanel({ sess }: { sess: Sess }) {
  ...
  return (
    <div className="space-y-3">
      ...
      <div className="text-xs text-muted">Luật: không kênh nào &gt;40% doanh thu một SKU · ≥70% sản lượng có hợp đồng trước · công nợ ≤15 ngày, &gt;30 ngày ngừng giao · giá sàn (bảng giá SAN) — bán dưới sàn cần GĐ duyệt.</div>
      ...
      {tab === "kenh" && <KenhPanel />}
      {tab === "don" && <div className="space-y-3">
        <div className="card"><h3 className="font-bold">Đơn hàng — chốt trước 15h → lệnh sản xuất/đóng gói hôm sau (SLA10)</h3><div className="flex flex-wrap gap-2 mt-2"><select className="input !w-56" value={of.partner_id ?? ""} onChange={(e) => setOf({ ...of, partner_id: e.target.value })}><option value="">Khách</option>{(partners.rows ?? []).filter((p) => String(p.kind).includes("KH")).map((p) => <option key={String(p.id)} value={String(p.id)}>{String(p.name)}</option>)}</select><select className="input !w-56" value={of.sku ?? ""} onChange={(e) => setOf({ ...of, sku: e.target.value })}><option value="">SKU</option>{(products.rows ?? []).filter((p) => ["THANH_PHAM","BAN_TP"].includes(String(p.kind))).map((p) => <option key={String(p.sku)} value={String(p.sku)}>{String(p.name)}</option>)}</select><input className="input !w-24" type="number" placeholder="SL" aria-label="SL" value={of.qty ?? ""} onChange={(e) => setOf({ ...of, qty: e.target.value })} /><input className="input !w-32" type="number" placeholder="Giá" aria-label="Giá" value={of.price ?? ""} onChange={(e) => setOf({ ...of, price: e.target.value })} /><input className="input !w-40" type="date" value={of.deliver_date ?? ""} onChange={(e) => setOf({ ...of, deliver_date: e.target.value })} /><button className="btn-primary !py-2" onClick={async () => { if (!of.partner_id || !of.sku || !of.qty) return; const j = await act("create_order", { partner_id: of.partner_id, channel: 1, deliver_date: of.deliver_date || null, lines: [{ sku: of.sku, qty: Number(of.qty), price: Number(of.price || 0) }] }); if (j.id) toast.ok(`Đơn ${j.id}`); else toast.err(j.error); setOf({}); orders.reload(); }}>Chốt đơn</button></div></div>
        <div className="card p-0 overflow-auto"><table className="tbl"><thead><tr><th className="pl-3">Đơn</th><th>Khách</th><th>Giao</th><th>Hàng</th><th className="text-right">Tiền</th><th>Chốt ≤15h</th><th>TT</th><th></th></tr></thead><tbody>{(orders.rows ?? []).map((o) => <tr key={String(o.id)}><td className="pl-3 font-mono">{String(o.id)}</td><td>{String(o.partner_name)}</td><td>{fmt.d(o.deliver_date)}</td><td className="text-sm">{(o.lines as { sku: string; qty: number }[]).map((l) => `${l.sku}×${l.qty}`).join(", ")}</td><td className="text-right">{fmt.vnd(o.total)}</td><td>{o.cutoff_ok ? <span className="b-grn">đúng</span> : <span className="b-yel">trễ</span>}</td><td><span className="b-gray">{String(o.status)}</span></td><td><OrderActions o={o} onDone={() => orders.reload()} /></td></tr>)}</tbody></table></div>
        <div className="text-xs text-muted">Luồng: Nhập → <b>Giữ hàng</b> (FEFO theo lô; thiếu → tự sinh lệnh SX cho D5) → Chốt → <b>Phiếu soạn</b> (in cho kho) → <b>Xuất kho</b> (từng lô, → GIAO) → Hoàn tất. Đơn từ sàn/portal cũng vào đây.</div></div>}
      {tab === "hd" && <div className="space-y-3">
        <div className="grid grid-cols-2 sm:grid-cols-3 gap-2"><KpiTile l="Sản lượng cam kết (HĐ hiệu lực)" v={fmt.n(cov.rows?.[0]?.committed)} /><KpiTile l="Đã bán quý này" v={fmt.n(cov.rows?.[0]?.sold_q)} /><KpiTile l="Luật ≥70% sản lượng có HĐ trước khi tăng đàn" v="theo dõi" sub="ghi HĐ đầy đủ để tính" /></div>
        <div className="card"><div className="flex flex-wrap gap-2"><select className="input !w-56" value={of.c_partner ?? ""} onChange={(e) => setOf({ ...of, c_partner: e.target.value })}><option value="">Đối tác</option>{(partners.rows ?? []).map((p) => <option key={String(p.id)} value={String(p.id)}>{String(p.name)}</option>)}</select><select className="input !w-40" value={of.c_kind ?? "BAO_TIEU"} onChange={(e) => setOf({ ...of, c_kind: e.target.value })}>{["BAO_TIEU","B2B","LIEN_KET_HO","NHUONG_QUYEN","NHAP_KHAU","XUAT_KHAU"].map((k) => <option key={k}>{k}</option>)}</select><select className="input !w-48" value={of.c_sku ?? ""} onChange={(e) => setOf({ ...of, c_sku: e.target.value })}><option value="">SKU</option>{(products.rows ?? []).map((p) => <option key={String(p.sku)} value={String(p.sku)}>{String(p.name)}</option>)}</select><input className="input !w-32" type="number" placeholder="SL cam kết" aria-label="SL cam kết" value={of.c_qty ?? ""} onChange={(e) => setOf({ ...of, c_qty: e.target.value })} /><input className="input !w-32" type="number" placeholder="Giá" aria-label="Giá" value={of.c_price ?? ""} onChange={(e) => setOf({ ...of, c_price: e.target.value })} /><input className="input !w-40" type="date" value={of.c_end ?? ""} onChange={(e) => setOf({ ...of, c_end: e.target.value })} /><button className="btn-secondary !py-2" onClick={async () => { if (!of.c_partner) return; await act("create_contract", { partner_id: of.c_partner, kind: of.c_kind ?? "BAO_TIEU", sku: of.c_sku || null, qty_committed: Number(of.c_qty || 0) || null, price: Number(of.c_price || 0) || null, start_date: new Date().toISOString().slice(0, 10), end_date: of.c_end || null }); setOf({}); contracts.reload(); cov.reload(); }}>+ Hợp đồng</button></div></div>
        <div className="card p-0 overflow-auto"><table className="tbl"><thead><tr><th className="pl-3">HĐ</th><th>Đối tác</th><th>Loại</th><th>SKU</th><th className="text-right">Cam kết</th><th className="text-right">Đã giao</th><th>Hết hạn</th></tr></thead><tbody>{(contracts.rows ?? []).map((c) => <tr key={String(c.id)}><td className="pl-3 font-mono">{String(c.id)}</td><td>{String(c.partner_name)}</td><td>{String(c.kind)}</td><td>{String(c.sku ?? "")}</td><td className="text-right">{fmt.n(c.qty_committed)}</td><td className="text-right font-bold">{fmt.n(c.delivered)}</td><td>{fmt.d(c.end_date)}</td></tr>)}</tbody></table></div></div>}
      {tab === "nn" && <div className="space-y-3">
        <div className="card"><h3 className="font-bold">Chăm sóc hộ / nhận nuôi (thu trước = nợ dịch vụ, đối ứng 30%; khách xem hồ sơ + QR con vật)</h3><div className="flex flex-wrap gap-2 mt-2"><select className="input !w-56" value={of.n_partner ?? ""} onChange={(e) => setOf({ ...of, n_partner: e.target.value })}><option value="">Khách</option>{(partners.rows ?? []).filter((p) => String(p.kind).includes("KH")).map((p) => <option key={String(p.id)} value={String(p.id)}>{String(p.name)}</option>)}</select><select className="input !w-40" value={of.n_kind ?? "NHAN_NUOI"} onChange={(e) => setOf({ ...of, n_kind: e.target.value })}>{["NHAN_NUOI","KY_GUI","DAU_TU","TANG"].map((k) => <option key={k}>{k}</option>)}</select><input className="input !w-64" placeholder="Mã con (F01-BO-…, cách nhau dấu phẩy)" aria-label="Mã con (F01-BO-…, cách nhau dấu phẩy)" value={of.n_animals ?? ""} onChange={(e) => setOf({ ...of, n_animals: e.target.value })} /><input className="input !w-32" type="number" placeholder="Phí/kỳ" aria-label="Phí/kỳ" value={of.n_fee ?? ""} onChange={(e) => setOf({ ...of, n_fee: e.target.value })} /><input className="input !w-24" type="number" placeholder="Tháng" aria-label="Tháng" value={of.n_m ?? ""} onChange={(e) => setOf({ ...of, n_m: e.target.value })} /><input className="input !w-36" type="number" placeholder="Thu trước" aria-label="Thu trước" value={of.n_pre ?? ""} onChange={(e) => setOf({ ...of, n_pre: e.target.value })} /><button className="btn-primary !py-2" onClick={async () => { if (!of.n_partner) return; const j = await act("create_custody", { partner_id: of.n_partner, kind: of.n_kind ?? "NHAN_NUOI", animal_ids: (of.n_animals ?? "").split(",").map((x) => x.trim()).filter(Boolean), fee: Number(of.n_fee || 0) || null, period_months: Number(of.n_m || 0) || null, prepaid: Number(of.n_pre || 0) || null }); if (j.id) toast.ok(`Hợp đồng ${j.id}`); else toast.err(j.error); setOf({}); custody.reload(); }}>+ Hợp đồng chăm sóc</button></div></div>
        <div className="card p-0 overflow-auto"><table className="tbl"><thead><tr><th className="pl-3">Mã</th><th>Khách</th><th>Loại</th><th>Con</th><th className="text-right">Phí/kỳ</th><th className="text-right">Thu trước</th><th>Bắt đầu</th><th>TT</th></tr></thead><tbody>{(custody.rows ?? []).map((c) => <tr key={String(c.id)}><td className="pl-3 font-mono">{String(c.id)}</td><td>{String(c.partner_name)}</td><td>{String(c.kind)}</td><td className="text-sm">{(c.animal_ids as string[]).map((a) => <Link key={a} className="underline mr-1" href={`/dan/${a}`}>{a.slice(-5)}</Link>)}</td><td className="text-right">{fmt.vnd(c.fee)}</td><td className="text-right">{fmt.vnd(c.prepaid)}</td><td>{fmt.d(c.start_date)}</td><td><span className="b-grn">{String(c.status)}</span></td></tr>)}</tbody></table></div>
        <div className="text-xs text-muted">Đợt sau: cổng khách hàng (ảnh mốc, camera theo khung giờ, chat SLA), báo cáo tháng tự sinh cho từng khách.</div></div>}
      {tab === "ban" && forms && ["worker","team_lead","director","accountant"].includes(sess.role) && <ThreeTap spec={{ ...forms.sale, onDone: () => { sales.reload(); recv.reload(); } }} />}
      {tab === "ban" && <div className="grid md:grid-cols-2 gap-3">
        <div className="card p-0 overflow-auto"><div className="px-3 py-2 font-bold bg-surface-2 rounded-t-2xl">Bán gần đây</div><table className="tbl"><thead><tr><th className="pl-3">Lúc</th><th>Khách</th><th>SKU</th><th className="text-right">SL</th><th className="text-right">Giá</th><th className="text-right">Tiền</th><th>Kênh</th><th>TT</th></tr></thead><tbody>{(sales.rows ?? []).map((s) => <tr key={String(s.id)} className={floor(String(s.sku)) && Number(s.price) < Number(floor(String(s.sku))) ? "bg-danger-soft-tok" : ""}><td className="pl-3">{fmt.dt(s.ts)}</td><td>{String(s.partner_name)}</td><td className="text-xs">{String(s.sku)}</td><td className="text-right">{fmt.n(s.qty)}</td><td className="text-right">{fmt.vnd(s.price)}{floor(String(s.sku)) && Number(s.price) < Number(floor(String(s.sku))) ? <span className="b-red ml-1">dưới sàn</span> : null}</td><td className="text-right font-bold">{fmt.vnd(s.amount)}</td><td>{String(s.channel)}</td><td>{s.paid ? <span className="b-grn">{String(s.payment)}</span> : <span className="b-yel">nợ</span>}</td></tr>)}</tbody></table></div>
        <div className="card"><h3 className="font-bold">Công nợ</h3><table className="tbl"><tbody>{(recv.rows ?? []).map((r) => <tr key={String(r.id)}><td>{String(r.name)}</td><td className="text-right">{fmt.vnd(r.unpaid)}</td><td className={Number(r.days) > 30 ? "text-danger-tok font-bold" : Number(r.days) > 15 ? "text-warning-tok" : ""}>{String(r.days)} ngày {Number(r.days) > 30 && "· NGỪNG GIAO"}</td></tr>)}</tbody></table>
          <h3 className="font-bold mt-3">Tuổi nợ</h3><table className="tbl"><tbody>{(aging.rows ?? []).map((r) => <tr key={String(r.partner_id)}><td>{String(r.name)}</td><td className="text-right">{fmt.vnd(r.d0_15 ?? 0)}</td><td className="text-right">{fmt.vnd(r.d16_30 ?? 0)}</td><td className={`text-right ${Number(r.d30p) > 0 ? "text-danger-tok font-bold" : ""}`}>{fmt.vnd(r.d30p ?? 0)}</td></tr>)}</tbody></table>
  ...
  return (
    <div className="space-y-3">
      ...
      {q && (<><div className="grid md:grid-cols-3 gap-3">
        <div className="card"><h3 className="font-bold">Lô</h3>{l.rows?.[0] ? <div className="text-sm">{String(l.rows[0].product_name)}<br />lô {String(l.rows[0].lot_no)} · NCC {String(l.rows[0].supplier_id ?? "nội bộ")}<br />hạn {fmt.d(l.rows[0].expiry_date)} · <span className={l.rows[0].status === "KHA_DUNG" ? "b-grn" : "b-red"}>{String(l.rows[0].status)}</span></div> : <div className="text-muted text-sm">Không thấy trong bảng lots (có thể là mã mẻ).</div>}</div>
        <div className="card"><h3 className="font-bold">← 1 bước lùi (nguyên liệu)</h3><DataBoundary loading={tree.loading} error={tree.error} empty={!back.length} onRetry={tree.reload} emptyText="Không có mẻ dùng lô này làm đầu ra."><ul className="text-sm">{back.map((r, i) => <li key={i}>{"—".repeat(Number(r.depth))} {String(r.input_lot)} <span className="text-muted">(mẻ {String(r.batch_code)})</span></li>)}</ul></DataBoundary></div>
        <div className="card"><h3 className="font-bold">→ 1 bước tiến (sản phẩm · khách)</h3><ul className="text-sm">{fwd.map((r, i) => <li key={i}>{"—".repeat(Number(r.depth))} {String(r.output_lot)} <span className="text-muted">(mẻ {String(r.batch_code)})</span></li>)}</ul><h4 className="font-semibold mt-2">Khách đã nhận lô này</h4><DataBoundary loading={cust.loading} error={cust.error} empty={!cust.rows?.length} onRetry={cust.reload} emptyText="Chưa bán lô này."><ul className="text-sm">{(cust.rows ?? []).map((c, i) => <li key={i}>{fmt.dt(c.ts)} · <b>{String(c.name)}</b> {String(c.phone ?? "")} · {fmt.n(c.qty)} {String(c.unit ?? "")}</li>)}</ul></DataBoundary>
          {recall && cust.rows && <div className="mt-2 text-xs text-muted">Thời gian dựng danh sách khách nhận: {((Date.now() - recall.start) / 1000).toFixed(1)}s (chuẩn ≤ 4h).</div>}</div>
      </div>
      {canRecall && <div className="card border-danger-tok">
        <div className="flex items-center gap-2 flex-wrap"><b className="text-danger-tok">🚨 Thu hồi lô</b>
          <span className="text-xs text-muted">Phát lệnh thu hồi: lô → THU_HOI (chặn xuất bán), tự sinh danh sách khách đã nhận + việc RECALL cho các bộ phận. Không hoàn tác.</span>
          {!rc && <button className="btn-secondary !py-2 !text-xs ml-auto" onClick={() => { setRc({ reason: "", severity: "NANG" }); setRcMsg(""); }}>＋ Lập lệnh thu hồi</button>}</div>
        ...
        {rcMsg && <div className="mt-2 text-sm text-brand">{rcMsg}</div>}
      </div>}
      </>)}
    </div>);
}

const Q = ({ q, v, ok }: { q: string; v: React.ReactNode; ok: boolean }) => <div className={`card ${ok ? "" : "border-danger-tok bg-danger-soft-tok"}`}><div className="text-sm">{q}</div><div className="text-2xl font-bold">{v}</div></div>;
export function SucKhoePanel({ sess }: { sess: Sess }) {
  ...
  return (
    <div className="space-y-4">
      <EpiPanel />
      <p className="text-sm text-muted">7 bộ câu chất vấn (chủ đầu tư/GĐ/kế toán hỏi định kỳ). Số đỏ = phải xử lý.</p>
      ...
      <div className="grid sm:grid-cols-3 gap-2"><Q q="% nhập bù (sự kiện vật nuôi / cho ăn) 30 ngày" v={`${String(r?.backfill_pct_animal ?? 0)}% / ${String(r?.backfill_pct_feed ?? 0)}%`} ok={Number(r?.backfill_pct_animal ?? 0) <= 5 && Number(r?.backfill_pct_feed ?? 0) <= 5} /><Q q="Công nhân 7 ngày không ghi gì (quay lại sổ giấy riêng?)" v={String(r?.workers_silent_7d ?? "…")} ok={Number(r?.workers_silent_7d ?? 0) === 0} /><div className="card"><div className="text-sm">Giờ ghi 7 ngày: rải theo ca hay dồn cục cuối ngày (ghi hồi ký)?</div><HourHistogram rows={rows} /><div className="text-xs text-muted">0h → 23h</div></div></div>
      ...
      <div className="text-xs text-muted">Trại {sess.farmId} · dữ liệu tính trực tiếp từ bảng sự kiện.</div>
    </div>);
}
