...
export default function SoLieuPanel({ sess, initialMetric, initialTab }: { sess: Sess; initialMetric?: string; initialTab?: string }) {
  ...
  return (<div className="space-y-3">
    <div className="flex gap-2">{[["kpi", "Chỉ số nghiệp vụ (catalog)"], ["any", "📈 Mọi trường dữ liệu (tự động)"]].map(([k, l]) => <button key={k} className={`px-4 py-2 rounded-xl font-semibold ${top === k ? "bg-brand-tok text-white" : "bg-surface border"}`} onClick={() => setTop(k as typeof top)}>{l}</button>)}</div>
...
function MetricExplorer({ sess, initialMetric }: { sess: Sess; initialMetric?: string }) {
  ...
  return (
    <div className="space-y-3">
      ...
      {drill && <div className="card p-0 overflow-auto"><div className="flex justify-between px-3 py-2 bg-surface-2 rounded-t-2xl"><b>Bản ghi gốc · {drill.t}{drill.dimval ? ` · ${drill.dimval}` : ""} ({drill.rows.length})</b><button className="underline text-sm" onClick={() => setDrill(null)}>đóng</button></div>
