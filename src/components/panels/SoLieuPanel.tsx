...
export default function SoLieuPanel({ sess, initialMetric, initialTab }: { sess: Sess; initialMetric?: string; initialTab?: string }) {
  ...
  return (<div className="space-y-3">
    <div className="flex gap-2">{[["kpi", "Chỉ số nghiệp vụ (catalog)"], ["any", "📈 Mọi trường dữ liệu (tự động)"]].map(([k, l]) => <button key={k} className={`px-4 py-2 rounded-xl font-semibold min-h-[40px] ${top === k ? "bg-brand-tok text-white" : "bg-surface border"}`} onClick={() => setTop(k as typeof top)}>{l}</button>)}</div>
  ...
  return (
    <div className="space-y-3">
      <div className="card grid sm:grid-cols-2 lg:grid-cols-6 gap-2 items-end">
        <div className="lg:col-span-2"><label className="text-xs text-muted">Chỉ số ({metrics.length})</label><select className="input" value={metric} onChange={(e) => { setMetric(e.target.value); setDim(""); }}>{groups.map((g) => <optgroup key={g} label={g}>{metrics.filter((x) => x.group === g).map((x) => <option key={x.code} value={x.code}>{x.name}</option>)}</optgroup>)}</select></div>
        <div><label className="text-xs text-muted">Kỳ</label><select className="input" value={bucket} onChange={(e) => setBucket(e.target.value)}><option value="day">Ngày</option><option value="week">Tuần</option><option value="month">Tháng</option><option value="quarter">Quý</option><option value="year">Năm</option></select></div>
        <div><label className="text-xs text-muted">Cắt theo</label><select className="input" value={dim} onChange={(e) => setDim(e.target.value)}><option value="">Tổng</option>{(m?.dims ?? []).map((d) => <option key={d} value={d}>{d}</option>)}</select></div>
        <div><label className="text-xs text-muted">Từ</label><input type="date" className="input" value={from} onChange={(e) => setFrom(e.target.value)} /></div>
        <div><label className="text-xs text-muted">Đến</label><input type="date" className="input" value={to} onChange={(e) => setTo(e.target.value)} /></div>
      </div>
      <div className="flex gap-2 items-center flex-wrap text-sm"><span>Nhanh:</span>{[["7 ngày", 7, "day"], ["30 ngày", 30, "day"], ["Quý này", 92, "week"], ["1 năm", 365, "month"], ["3 năm", 1095, "quarter"]].map(([l, d, b]) => <button key={String(l)} className="btn-secondary !py-1 !px-3 !text-sm" onClick={() => { setFrom(new Date(Date.now() - Number(d) * 86400e3).toISOString().slice(0, 10)); setTo(new Date().toISOString().slice(0, 10)); setBucket(String(b)); }}>{l}</button>)}
        <span className="ml-auto flex gap-1 items-center">{[["bar", "Cột"], ["line", "Đường"], ["cum", "Lũy kế"]].map(([k, l]) => <button key={k} className={`btn-secondary !py-1 !px-3 !text-sm ${chart === k ? "!bg-brand-tok !text-white" : ""}`} onClick={() => setChart(k as typeof chart)}>{l}</button>)}<label className="ml-2 flex items-center gap-1"><input type="checkbox" checked={compare} onChange={(e) => setCompare(e.target.checked)} /> So kỳ trước</label></span></div>
      <div className="card"><div className="flex flex-wrap justify-between items-baseline gap-2"><h3 className="font-bold">{m?.name} <span className="text-sm font-normal text-muted">({m?.unit}) · {sess.farmId}</span></h3><div className="text-sm">Tổng kỳ: <b>{fmt.n(total, 1)}</b> {m?.unit}{compare && prevTotal > 0 && <span className={total >= prevTotal ? "text-brand" : "text-danger-tok"}> · {total >= prevTotal ? "▲" : "▼"} {fmt.n(((total - prevTotal) / prevTotal) * 100, 1)}% so kỳ trước ({fmt.n(prevTotal, 1)})</span>}{norm != null && <span className="text-muted"> · định mức ≈ {fmt.n(norm)}/kỳ</span>}</div></div>
        {loading && <div className="ui-skel py-10"><span className="sr-only">Đang tải…</span></div>}
        {!loading && err && <div role="alert" className="text-danger-tok">{err}</div>}
        {!loading && !rows.length && !err && <div className="text-muted py-10 text-center">Chưa có dữ liệu trong khoảng này.</div>}
        ...
        <div className="text-xs text-muted mt-1">Bấm vào cột/điểm → danh sách bản ghi gốc (ai ghi, lúc nào). Đường xám đứt = định mức (NORM). "So kỳ trước" vẽ kỳ liền trước cùng độ dài.</div></div>
      {drill && <div className="card p-0 overflow-auto"><div className="flex justify-between px-3 py-2 bg-surface-2 rounded-t-2xl"><b>Bản ghi gốc · {drill.t}{drill.dimval ? ` · ${drill.dimval}` : ""} ({drill.rows.length})</b><button className="underline text-sm py-2 -my-2" onClick={() => setDrill(null)}>đóng</button></div>
        <table className="tbl"><thead><tr>{Object.keys(drill.rows[0] ?? {}).slice(0, 12).map((k) => <th key={k} className="pl-2">{k}</th>)}</tr></thead><tbody>{drill.rows.slice(0, 200).map((r, i) => <tr key={i}>{Object.keys(drill.rows[0] ?? {}).slice(0, 12).map((k) => <td key={k} className="pl-2 text-sm">{k === "ts" ? fmt.dt(r[k]) : typeof r[k] === "object" ? JSON.stringify(r[k]).slice(0, 60) : String(r[k] ?? "")}</td>)}</tr>)}</tbody></table>{drill.rows?.length === 0 && <div className="p-3 text-muted">Không có bản ghi.</div>}</div>}
      {rows.length > 0 && <div className="card p-0 overflow-auto"><div className="flex justify-between px-3 py-2 bg-surface-2 rounded-t-2xl"><b>Bảng số liệu</b><button className="underline text-sm py-2 -my-2" onClick={() => { const csv = "﻿ky,chieu,gia_tri\n" + rows.map((r) => `${r.t},${r.dim ?? ""},${r.v}`).join("\n"); const a = document.createElement("a"); a.href = URL.createObjectURL(new Blob([csv], { type: "text/csv" })); a.download = `${metric}-${bucket}-${from}_${to}.csv`; a.click(); }}>⬇ CSV</button></div><table className="tbl"><thead><tr><th className="pl-3">Kỳ</th>{keys.map((k) => <th key={k} className="text-right">{k}</th>)}</tr></thead><tbody>{data.map((d, i) => <tr key={i} className="cursor-pointer hover:bg-green-50" onClick={() => onPoint(d)}><td className="pl-3">{String(d.t)}</td>{keys.map((k) => <td key={k} className="text-right">{d[k] != null ? fmt.n(d[k], 1) : ""}</td>)}</tr>)}</tbody></table></div>}
    </div>);
}
