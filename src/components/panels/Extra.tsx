...
export function SoDoPanel({ sess }: { sess: Sess }) {
  ...
  return (
    <div className="space-y-3">
      <div className="flex items-center gap-2 text-sm"><span>Sơ đồ khu trại {sess.farmId} — màu: <span className="b-grn">bình thường</span> <span className="b-yel">có cần chú ý</span> <span className="b-red">≥3 cần chú ý</span> <span className="b-gray">trống</span></span>{canW && <button className={`btn-secondary !py-1 !px-3 !text-sm ml-auto ${edit ? "!bg-brand-tok !text-white" : ""}`} onClick={() => setEdit(!edit)}>{edit ? "✓ Xong sắp xếp" : "Sắp xếp (kéo thả)"}</button>}</div>
...
export function SavedViews({ scope, current, onPick }: { scope: string; current: Record<string, unknown>; onPick: (v: Record<string, unknown>) => void }) {
  ...
  return <div className="flex flex-wrap gap-1 items-center text-sm">{promptElement}<span className="text-muted">Bộ lọc lưu:</span>{views.map((x) => <span key={x.name} className="b-gray cursor-pointer" onClick={() => onPick(x.v)}>{x.name} <button className="ml-1 text-danger-tok" onClick={(e) => { e.stopPropagation(); const nv = views.filter((y) => y.name !== x.name); setViews(nv); localStorage.setItem(key, JSON.stringify(nv)); }}>×</button></span>)}<button className="underline" onClick={save}>+ lưu bộ lọc hiện tại</button></div>;
}
