    ...
    <div className="space-y-4">
      <div className="card"><div className="flex flex-wrap items-center gap-2"><h3 className="font-bold">P&L phân hệ (CC) — doanh thu ngoài + nội bộ 70% − vật tư trực tiếp − nội bộ − chi phí cố định = EBITDA</h3><select className="input !w-40 ml-auto" value={cur ?? ""} onChange={(e) => setM(e.target.value)}>{months.map((x) => <option key={x} value={x}>{x.slice(0, 7)}</option>)}</select></div>
        <div className="overflow-auto mt-2"><table className="tbl"><thead><tr><th>CC</th><th className="text-right">DT ngoài</th><th className="text-right">DT nội bộ</th><th className="text-right">Vật tư</th><th className="text-right">Mua nội bộ</th><th className="text-right">Cố định</th><th className="text-right">EBITDA</th></tr></thead><tbody>
          {rows.map((r) => <tr key={String(r.cc)} className={Number(r.ebitda) < 0 ? "bg-danger-soft-tok" : ""}><td className="font-semibold">{String(r.cc)}</td><td className="text-right">{fmt.vnd(r.revenue_external)}</td><td className="text-right">{fmt.vnd(r.revenue_internal)}</td><td className="text-right">{fmt.vnd(r.material_cost)}</td><td className="text-right">{fmt.vnd(r.internal_cost)}</td><td className="text-right">{fmt.vnd(r.fixed_cost)}</td><td className={`text-right font-bold ${Number(r.ebitda) < 0 ? "text-danger-tok" : "text-brand"}`}>{fmt.vnd(r.ebitda)}</td></tr>)}
        ...
        <div className="text-xs text-muted mt-1">Phân hệ lỗ 3 quý liên tiếp → tái cấu trúc/cắt (trừ hạ tầng trục). Vật tư định giá theo giá vốn lô; nội bộ = 70% giá sàn. Chi phí cố định (lương, khấu hao, điện nước, thuê ngoài) nhập theo tháng bên dưới.</div></div>
      <div className="grid md:grid-cols-2 gap-3">
        <div className="card"><h3 className="font-bold">Chi phí cố định theo CC / tháng</h3>
          {canW && <div className="flex flex-wrap gap-2 mt-2"><input type="date" className="input !w-40" value={f.month} onChange={(e) => setF({ ...f, month: e.target.value })} /><select className="input !w-32" value={f.cc ?? ""} onChange={(e) => setF({ ...f, cc: e.target.value })}><option value="">CC</option>{["CC-BO","CC-GA","CC-RAS","CC-TRUN","CC-D5","CC-TT","CC-CB","CC-KD","CC-DL","CC-HC","CC-CN","CC-VIEN"].map((c) => <option key={c}>{c}</option>)}</select><select className="input !w-36" value={f.kind} onChange={(e) => setF({ ...f, kind: e.target.value })}>{["LUONG","KHAU_HAO","DIEN_NUOC","THUE_NGOAI","KHAC"].map((k) => <option key={k}>{k}</option>)}</select><input className="input !w-36" type="number" placeholder="Số tiền" aria-label="Số tiền" value={f.amount ?? ""} onChange={(e) => setF({ ...f, amount: e.target.value })} /><button className="btn-secondary !py-2" onClick={async () => { if (!f.cc || !f.amount) return; await act("add_fixed_cost", { month: f.month.slice(0, 7) + "-01", cost_center: f.cc, kind: f.kind, amount: Number(f.amount) }); fixed.reload(); pl.reload(); }}>Lưu</button></div>}
  ...
  return (
    <div className="space-y-3">
      ...
      <div className="card p-0 overflow-auto"><table className="tbl"><thead><tr><th className="pl-3">Nhân sự</th><th className="text-right">Điểm</th><th className="text-right">Thưởng lớp 2</th><th>Chi tiết (giá trị / mục tiêu / điểm)</th></tr></thead><tbody>
        {(k.rows ?? []).map((r) => <tr key={String(r.staff_id)} className={Number(r.score) < 60 ? "bg-danger-soft-tok" : Number(r.score) >= 95 ? "bg-brand-soft" : ""}><td className="pl-3 font-semibold">{String(r.full_name)}<div className="text-xs text-muted font-normal">{String(r.position ?? "")}</div></td><td className="text-right text-2xl font-bold">{String(r.score)}</td><td className="text-right font-bold">{String(r.bonus_pct)}%</td><td className="text-xs">{Object.entries((r.detail as Record<string, { v: number; t: number; s: number }>) ?? {}).map(([c, d]) => <span key={c} className="b-gray mr-1">{c.replace("KPI-", "")}: {fmt.n(d.v, 1)}/{d.t} → {d.s}</span>)}</td></tr>)}
        {k.rows?.length === 0 && <tr><td colSpan={4} className="p-3 text-muted">Chưa chấm tháng này — bấm "Chấm KPI".</td></tr>}</tbody></table></div>
      <div className="card"><h3 className="font-bold">Điểm KPI trung bình theo tháng</h3><div className="flex gap-2 flex-wrap mt-1">{(hist.rows ?? []).map((h) => <span key={String(h.month)} className="b-gray">{String(h.month).slice(0, 7)}: {String(h.score)}</span>)}</div><div className="text-xs text-muted mt-1">Mọi điểm drill được về bản ghi (số bản ghi/ngày, checklist xanh %, sai số mẻ, việc đúng hạn, % nhập bù). Người lao động xem được điểm của mình.</div></div>
    </div>);
}
...
export function SoDoPanel({ sess }: { sess: Sess }) {
  ...
  return (
    <div className="space-y-3">
      <div className="flex items-center gap-2 text-sm"><span>Sơ đồ khu trại {sess.farmId} — màu: <span className="b-grn">bình thường</span> <span className="b-yel">có cần chú ý</span> <span className="b-red">≥3 cần chú ý</span> <span className="b-gray">trống</span></span>{canW && <button className={`btn-secondary !py-2 !px-3 !text-sm ml-auto ${edit ? "!bg-brand-tok !text-white" : ""}`} onClick={() => setEdit(!edit)}>{edit ? "✓ Xong sắp xếp" : "Sắp xếp (kéo thả)"}</button>}</div>
      <div className="card overflow-auto"><div className="grid gap-2" style={{ gridTemplateColumns: `repeat(${cols}, minmax(120px, 1fr))` }}>
        {Array.from({ length: rowsN * cols }, (_, i) => { const x = i % cols, y = Math.floor(i / cols); const r = cells.get(`${x},${y}`);
          return <div key={i} draggable={edit && !!r} onDragStart={() => setDrag(String(r?.id))} onDragOver={(e) => edit && e.preventDefault()} onDrop={async () => { if (edit && drag) { await act("set_grid", { id: drag, x, y }); setDrag(null); g.reload(); } }}
            className={`rounded-xl border p-2 min-h-[84px] ${!r ? "border-dashed border-line" : Number(r.attention) >= 3 ? "bg-danger-soft-tok border-danger-tok" : Number(r.attention) > 0 ? "bg-warning-soft-tok border-amber-300" : Number(r.head_individual) + Number(r.head_group) > 0 ? "bg-brand-soft border-green-300" : "bg-surface-2"}`}>
            {r && <Link href={`/dan?location=${r.id}`} className="block"><div className="font-semibold text-sm leading-tight">{String(r.name)}</div><div className="text-xs text-muted">{String(r.kind)}</div><div className="text-sm mt-1">{Number(r.head_individual) > 0 && <b>{String(r.head_individual)} con</b>}{Number(r.head_group) > 0 && <span> · {fmt.n(r.head_group)} nhóm</span>}</div>{Number(r.attention) > 0 && <div className="text-xs text-warning-tok">⚠ {String(r.attention)} cần chú ý</div>}{Number(r.open_tasks) > 0 && <div className="text-xs">{String(r.open_tasks)} việc</div>}</Link>}
          </div>; })}
      </div></div>
      <div className="text-xs text-muted">Bản đồ GIS thật (polygon lô, geofence) ở Ray B; sơ đồ này đủ để "nhìn một cái biết chỗ nào có việc".</div>
    </div>);
}
...
export function KhachMessages() {
  ...
  return (<div className="card"><h3 className="font-bold">Tin nhắn khách chăm sóc hộ (SLA trả lời ≤ 24h)</h3>
    ...
    {(m.rows ?? []).filter((x) => x.from_customer).slice(0, 30).map((x) => <div key={String(x.id)} className="border-b py-2 text-sm"><div><b>{String(x.partner_name ?? x.contract_id)}</b> · {fmt.dt(x.ts)} {x.animal_id ? `· ${x.animal_id}` : ""}{x.replied_at ? <span className="b-grn ml-1">đã trả lời</span> : <span className="b-yel ml-1">chờ</span>}</div><div>{String(x.body)}</div>{!x.replied_at && <div className="flex gap-2 mt-1"><input className="input !py-1" placeholder="Trả lời…" aria-label="Trả lời…" value={reply[String(x.id)] ?? ""} onChange={(e) => setReply({ ...reply, [String(x.id)]: e.target.value })} /><button className="btn-primary !py-2 !px-3 !text-sm" onClick={async () => { await act("reply_customer", { reply_to: x.id, contract_id: x.contract_id, animal_id: x.animal_id, body: reply[String(x.id)] }); m.reload(); }}>Gửi</button></div>}</div>)}
    </DataBoundary></div>);
}
...
export function SavedViews({ scope, current, onPick }: { scope: string; current: Record<string, unknown>; onPick: (v: Record<string, unknown>) => void }) {
  ...
  return <div className="flex flex-wrap gap-1 items-center text-sm">{promptElement}<span className="text-muted">Bộ lọc lưu:</span>{views.map((x) => <span key={x.name} className="b-gray cursor-pointer" onClick={() => onPick(x.v)}>{x.name} <button className="ml-1 text-danger-tok py-1 -my-1 px-1 -mx-1" onClick={(e) => { e.stopPropagation(); const nv = views.filter((y) => y.name !== x.name); setViews(nv); localStorage.setItem(key, JSON.stringify(nv)); }}>×</button></span>)}<button className="underline py-2 -my-2" onClick={save}>+ lưu bộ lọc hiện tại</button></div>;
}
