...
export function KpiTile({ l, v, sub, warn }: { l: string; v: React.ReactNode; sub?: React.ReactNode; warn?: "red" | "yel" | null }) {
  return <div className={`kpi ${warn === "red" ? "border-danger-tok bg-danger-soft-tok" : warn === "yel" ? "border-amber-300 bg-warning-soft-tok" : ""}`}><div className="l">{l}</div><div className="v">{v}</div>{sub && <div className="text-xs text-muted">{sub}</div>}</div>;
}
  ...
  return (
    <div className="space-y-4">
      <div className="flex flex-wrap items-center gap-2 text-sm">
        <span className="text-muted">Trại <b>{sess.farmId}</b> · {new Date().toLocaleDateString("vi-VN", { weekday: "long", day: "2-digit", month: "2-digit" })}</span>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
        <KpiTile l="Cảnh báo ĐỎ chưa xử lý" v={reds.length} warn={reds.length ? "red" : null} sub={<Link className="underline py-2 -my-2" href="/canh-bao">xem</Link>} />
        ...
        <KpiTile l="Đối soát lệch (kỳ gần nhất)" v={(recon.rows ?? []).filter((r) => ["DO","VANG"].includes(String(r.status))).length} warn={(recon.rows ?? []).some((r) => r.status === "DO") ? "red" : (recon.rows ?? []).some((r) => r.status === "VANG") ? "yel" : null} sub={<Link className="underline py-2 -my-2" href="/doi-soat">RC1–RC12</Link>} />
      ...
      <div className="card"><h3 className="font-bold">6 dòng cân bằng vật chất tuần này (kg)</h3>
        <div className="grid grid-cols-2 sm:grid-cols-6 gap-2 mt-2 text-center">
          {[["Sinh khối vào (ruộng)", bal?.biomass_in_kg], ["Mua ngoài (K2)", bal?.purchased_in_kg], ["Thức ăn tiêu thụ", bal?.feed_used_kg], ["Sản phẩm ra", bal?.products_out_kg], ["Phân sinh ra (ước)", bal?.manure_est_kg], ["Phân xử lý → hữu cơ ra", `${fmt.n(bal?.manure_treated_kg)} → ${fmt.n(bal?.organic_out_kg)}`]].map(([l, v]) => <div key={String(l)} className="rounded-xl bg-surface-2 p-2"><div className="text-xs text-muted">{String(l)}</div><div className="font-bold text-lg">{typeof v === "string" ? v : fmt.n(v)}</div></div>)}
        </div><div className="text-xs text-muted mt-1">Dòng không nơi nhận = rò rỉ tiền (rà thứ 6). Phân sinh ra = đàn × định mức 15 kg/con/ngày (NORM).</div></div>
      <div className="grid md:grid-cols-2 gap-3">
        <div className="card"><h3 className="font-bold">Đỏ trong tuần</h3><DataBoundary loading={w.loading} error={w.error} empty={!(o?.red_week as unknown as unknown[] | null)?.length} onRetry={w.reload} emptyText="Không có đỏ 7 ngày qua"><ul className="text-sm mt-1 space-y-1">{((o?.red_week as unknown as Record<string, unknown>[] | null) ?? []).map((a, i) => <li key={i} className="border-l-4 border-danger-tok pl-2">{fmt.dt(a.ts)} · <b>{String(a.rule_code)}</b> {String(a.subject ?? "")}</li>)}</ul></DataBoundary></div>
        <div className="card"><h3 className="font-bold">Đề nghị chi chờ duyệt (2 chữ ký &gt;20tr · SMS chủ &gt;50tr)</h3>
          <DataBoundary loading={exp.loading} error={exp.error} empty={!pendingExp.length} onRetry={exp.reload} emptyText="Không có">
          {pendingExp.map((e) => <div key={String(e.id)} className="flex items-center gap-2 border-b py-1 text-sm"><span className="font-mono">{String(e.id)}</span><span className="flex-1">{String(e.purpose)} · {String(e.by_name)}</span><b>{fmt.vnd(e.amount)}</b><span className="b-gray">{String(e.status)}</span>{["team_lead","tech_head","director","owner"].includes(sess.role) && <button className="btn-primary !py-1 !px-2 !text-sm" onClick={async () => { const j = await act("approve_expense", { id: e.id, approve: true }); if (j.error) toast.err(j.error); else toast.ok(`→ ${j.status}${j.note ? " · " + j.note : ""}${j.sms_owner ? " · SMS chủ đầu tư" : ""}`); exp.reload(); }}>Ký</button>}</div>)}
          </DataBoundary>
          <ExpenseForm onDone={() => exp.reload()} /></div>
        <div className="card"><h3 className="font-bold">Công nợ theo khách</h3><table className="tbl"><tbody>{(recv.rows ?? []).map((r) => <tr key={String(r.id)}><td>{String(r.name)}</td><td className="text-right">{fmt.vnd(r.unpaid)}</td><td className={Number(r.days) > 30 ? "text-danger-tok font-bold" : Number(r.days) > 15 ? "text-warning-tok" : ""}>{String(r.days)} ngày</td></tr>)}</tbody></table></div>
...
function ExpenseForm({ onDone }: { onDone: () => void }) {
  ...
  return <div className="flex flex-wrap gap-2 mt-2"><input className="input !w-40" type="number" placeholder="Số tiền" aria-label="Số tiền" value={amt} onChange={(e) => setAmt(e.target.value)} /><input className="input !w-32" placeholder="CC-BO…" aria-label="CC-BO…" value={cc} onChange={(e) => setCc(e.target.value)} /><input className="input flex-1 !min-w-[200px]" placeholder="Mục đích chi" aria-label="Mục đích chi" value={p} onChange={(e) => setP(e.target.value)} /><button className="btn-secondary !py-2" onClick={async () => { if (!amt || !p) return; await act("create_expense", { amount: Number(amt), purpose: p, cost_center: cc || null }); setAmt(""); setP(""); onDone(); }}>Đề nghị chi</button></div>;
}
...
export function KttPanel({ sess }: { sess: Sess }) {
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid md:grid-cols-2 gap-3">
        <div className="card"><div className="flex items-center gap-2"><h3 className="font-bold">Duyệt checklist ca (không tự duyệt của mình)</h3><button className="btn-secondary !py-1 !px-3 !text-sm ml-auto" onClick={async () => { const j = await act("bulk_approve_checklists", {}); toast.ok(`Đã duyệt ${j.n ?? 0} checklist xanh`); cl.reload(); }}>Duyệt hàng loạt (xanh)</button></div>{(cl.rows ?? []).map((c) => <div key={String(c.id)} className="flex items-center gap-2 border-b py-1 text-sm"><b>{String(c.sop_code)}</b><span>{String(c.shift)}</span><span className={c.all_green ? "b-grn" : "b-yel"}>{c.all_green ? "xanh hết" : "có bước không đạt"}</span><span className="flex-1 text-muted">{String(c.created_by)} {String(c.note ?? "")}</span>{c.approved_by ? <span className="b-gray">đã duyệt</span> : <button className="btn-primary !py-1 !px-2 !text-sm" onClick={async () => { const j = await act("approve_checklist", { id: c.id }); if (j.error) toast.err(j.error); cl.reload(); }}>Duyệt</button>}</div>)}{cl.rows?.length === 0 && <div className="text-muted text-sm">Chưa có checklist hôm nay</div>}</div>
        <div className="card"><h3 className="font-bold">Đối chiếu chéo: cho ăn hôm nay (cân xe vs kế hoạch)</h3><table className="tbl"><thead><tr><th>Lúc</th><th>Đàn</th><th>Cữ</th><th className="text-right">KH</th><th className="text-right">Thực</th><th>Ảnh</th><th>Người</th></tr></thead><tbody>{(feed.rows ?? []).map((f) => <tr key={String(f.id)}><td>{fmt.dt(f.ts)}</td><td>{String(f.dest_group_id ?? f.dest_location_id ?? "")}</td><td>{String(f.meal ?? "")}</td><td className="text-right">{fmt.n(f.planned_kg)}</td><td className={`text-right font-bold ${f.planned_kg && Math.abs(Number(f.qty_kg) - Number(f.planned_kg)) / Number(f.planned_kg) > 0.02 ? "text-danger-tok" : ""}`}>{fmt.n(f.qty_kg)}</td><td>{Array.isArray(f.photo_urls) && (f.photo_urls as string[]).length ? "📷" : <span className="text-danger-tok">thiếu</span>}</td><td className="text-xs">{String(f.by_name)}</td></tr>)}</tbody></table></div>
        <div className="card"><h3 className="font-bold">PO chờ duyệt (nhập kho phải có PO)</h3><DataBoundary loading={po.loading} error={po.error} empty={!pendingPO.length} onRetry={po.reload} emptyText="Không có">{pendingPO.map((p) => <div key={String(p.id)} className="flex items-center gap-2 border-b py-1 text-sm"><span className="font-mono">{String(p.id)}</span><span className="flex-1">{String(p.supplier_name)} · {(p.lines as { sku: string; qty: number }[]).map((l) => `${l.sku}×${l.qty}`).join(", ")}</span><b>{fmt.vnd(p.total)}</b><button className="btn-primary !py-1 !px-2 !text-sm" onClick={async () => { const j = await act("po_status", { id: p.id, status: "DUYET" }); if (j.error) toast.err(j.error); po.reload(); }}>Duyệt</button></div>)}</DataBoundary><PoForm onDone={() => po.reload()} /></div>
      ...
      <div className="text-sm text-muted">Vai {sess.role}: được sửa RECIPE/SOP có phiên bản, duyệt điều chỉnh tồn (trang Kho), kiểm kê đột xuất, không được vừa ghi vừa duyệt cùng bản ghi.</div>
    </div>);
}
function PoForm({ onDone }: { onDone: () => void }) {
  ...
  return <div className="flex flex-wrap gap-2 mt-2"><select className="input !w-48" value={sup} onChange={(e) => setSup(e.target.value)}><option value="">NCC</option>{(partners.rows ?? []).filter((p) => String(p.kind).includes("NCC")).map((p) => <option key={String(p.id)} value={String(p.id)}>{String(p.name)}</option>)}</select><select className="input !w-48" value={sku} onChange={(e) => setSku(e.target.value)}><option value="">Hàng</option>{(products.rows ?? []).map((p) => <option key={String(p.sku)} value={String(p.sku)}>{String(p.name)}</option>)}</select><input className="input !w-24" type="number" placeholder="SL" aria-label="SL" value={qty} onChange={(e) => setQty(e.target.value)} /><input className="input !w-28" type="number" placeholder="Giá" aria-label="Giá" value={price} onChange={(e) => setPrice(e.target.value)} /><button className="btn-secondary !py-2" onClick={async () => { if (!sup || !sku || !qty) return; await act("create_po", { supplier_id: sup, lines: [{ sku, qty: Number(qty), price: Number(price || 0) }] }); setQty(""); onDone(); }}>Tạo PO</button></div>;
}
export function HourHistogram({ rows }: { rows: Record<string, unknown>[] }) {
  ...
  return <div className="flex items-end gap-0.5 h-24 mt-2">{h.map((v, i) => <div key={i} title={`${i}h: ${v}`} className="flex-1 bg-brand-tok rounded-t" style={{ height: `${(v / max) * 100}%` }} />)}</div>;
}
...
export function HqPanel() {
  ...
  return (
    <div className="space-y-4">
      <div className="card p-0 overflow-auto"><div className="px-3 py-2 font-bold bg-surface-2 rounded-t-2xl">So sánh trại — công ty mẹ ITRAN FARM (bấm vào trại để chuyển ngữ cảnh)</div>
        <table className="tbl"><thead><tr><th className="pl-3">Trại</th><th>Vùng</th><th className="text-right">Đàn</th><th className="text-right">Đậu thai %</th><th className="text-right">Bê/nái</th><th className="text-right">Ngày-tồn ủ</th><th className="text-right">Sai số mẻ %</th><th className="text-right">Đẻ %</th><th className="text-right">Việc quá hạn</th><th className="text-right">Đỏ</th><th className="text-right">Giấy chưa số hóa</th><th className="text-right">DT tháng</th><th className="text-right">Công nợ</th></tr></thead><tbody>
          {(k.rows ?? []).map((r) => { const s = (f.rows ?? []).find((x) => x.id === r.farm_id); return <tr key={String(r.farm_id)}><td className="pl-3 font-bold"><button className="underline py-2 -my-2" onClick={async () => { await fetch("/api/auth/switch-farm", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ farm_id: r.farm_id }) }); location.href = "/gd"; }}>{String(r.farm_id)}</button><div className="text-xs font-normal text-muted">{String(r.name)}</div></td><td>{String(r.region_id ?? "")}</td><td className="text-right">{fmt.n(s?.head_count)}</td><td className="text-right">{r.conception_pct != null ? fmt.n(r.conception_pct) : "—"}</td><td className="text-right">{r.weaned_per_dam != null ? fmt.n(r.weaned_per_dam, 2) : "—"}</td><td className={`text-right ${r.days_silage != null && Number(r.days_silage) < 45 ? "text-warning-tok font-bold" : ""}`}>{r.days_silage != null ? fmt.n(r.days_silage) : "—"}</td><td className="text-right">{r.feed_err_pct != null ? fmt.n(r.feed_err_pct, 1) : "—"}</td><td className="text-right">{r.lay_pct != null ? fmt.n(r.lay_pct, 1) : "—"}</td><td className="text-right">{String(r.overdue_tasks)}</td><td className={`text-right ${Number(r.red_alerts) ? "text-danger-tok font-bold" : ""}`}>{String(r.red_alerts)}</td><td className="text-right">{String(r.paper_pending)}</td><td className="text-right">{fmt.vnd(r.revenue_mtd)}</td><td className="text-right">{fmt.vnd(r.receivable)}</td></tr>; })}
        </tbody></table></div>
      <div className="grid md:grid-cols-3 gap-3">
        ...
        <div className="card"><h3 className="font-bold">Chuẩn toàn hệ (ORG)</h3><p className="text-sm">SKU, SOP, KPI, alert, RC, định mức là dữ liệu cấp công ty — trại kế thừa; sửa có phiên bản, đẩy xuống trại ký nhận (SOP_DISTRIBUTION).</p><Link className="underline text-sm py-2 -my-2" href="/sop">Thư viện SOP →</Link></div>
      </div>
    </div>);
}
  ...
  return (
    <div className="space-y-4">
      <div className="card"><h3 className="font-bold">Trung tâm xuất dữ liệu — cho kiểm toán, cơ quan thuế, cơ quan nông nghiệp, đối tác, chủ đầu tư</h3>
        ...
        <div className="text-xs text-muted mt-2">CSV UTF-8 (BOM, mở Excel được). ZIP kèm MANIFEST sha256 để chứng minh toàn vẹn. EPCIS 2.0 JSON-LD theo lô: <code>/api/exports/epcis?lot=…</code>. Mọi bảng đơn lẻ: <code>/api/exports/table:tên_bảng</code>.</div></div>
      <div className="grid md:grid-cols-2 gap-3">
        ...
        <div className="card"><h3 className="font-bold">Đối chiếu giấy–số</h3><div className="text-sm">Phiếu đã chụp: {paper.rows?.length ?? 0} · chưa số hóa: {(paper.rows ?? []).filter((p) => !p.digitized).length}</div><Link className="underline text-sm py-2 -my-2" href="/giay">Mở màn đối chiếu (ảnh gốc cạnh dữ liệu) →</Link></div>
      </div>
    </div>);
}
