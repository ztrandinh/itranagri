...
export async function dispatchEvents(limit = 500): Promise<number> {
  ...
  for (const e of evs) {
    ...
    else if (e.topic === "order.created") { recips = await resolveRecipients(farm ?? "F01", ["worker:A9", "worker:A8", "director", "accountant"]); level = Number(pl.unmapped ?? 0) > 0 ? "VANG" : "INFO"; title = `Đơn mới ${pl.id} (${pl.source ?? "nội bộ"}) · ${Number(pl.total ?? 0).toLocaleString("vi-VN")} đ`; body = `${pl.partner ?? ""}${Number(pl.unmapped ?? 0) > 0 ? ` · ${pl.unmapped} dòng chưa ánh xạ SKU — Kinh doanh xử lý` : ""} · Kho soạn hàng FEFO · Kế toán theo dõi thu`; link = "/ban-hang?tab=don"; sourceId = String(pl.id); }
