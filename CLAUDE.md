...
Job đêm: `POST /api/jobs/all?farm=F01` với header `x-job-key` (recon RC1–RC12 + alerts + tasks v2 + agg_daily/snapshot + feed_plans); `/api/jobs/agg?days=35` backfill. Trang: /trang-chu (khu vực) /ca /dan /canh-tac /kho /giay /gd /ktt /hq (+?tab=cty, /hq/trai/{id}) /audit /doi-soat /canh-bao (+?tab=caidat) /sop /ban-hang (+crm/pos/kenh) /du-lich /truy-xuat /suc-khoe /so-lieu (+?tab=any) /ke-hoach /thu-y /nhan-su /thiet-bi /ke-toan /to-chuc (phòng ban·quy trình·khai báo/chạy quy trình·I/O·event bus·hồ sơ) /doi-tuong /quan-tri (thêm/sửa/gỡ/nhập CSV/xuất/lịch sử mọi bảng) /xem/{type}/{id} (360 mọi đối tượng, ô 🔍 header) /tai-khoan /trace/{qr}. Job dispatch thông báo: `POST /api/jobs/dispatch?farm=F01` (x-job-key) mỗi phút hoặc trong /jobs/all. Tài khoản dev: owner/gd/ktt-cn/ks-cn/tn-bo/a1…a11/audit/kt · PIN 1234.

## Cấu trúc
`supabase/migrations/*.sql` (schema thuần SQL 0001–0024: 0009 quy mô/agg, 0012 kế hoạch/đơn/HĐ/thú y/nhận nuôi, 0014 P&L/KPI/portal, 0016 event_bus/notifications/create_farm, 0017 hồ sơ trại/facilities/audit_log/R&D/XNK/franchise/documents/webhooks, 0018 canh tác(PHI)/thu hoạch/CRM/POS/du lịch/records_catalog, 0020 phòng ban/quy trình/event_topics, 0021 đối tượng (roles/positions/species/classes/crops/product_kinds), 0022 kiểm kê 65 quy trình, 0023 workflow engine (khai báo/xuất bản/chạy quy trình), 0024 … 0042 thư viện SOP Quyển 3 §15 (81 L2 SOP-XX-NN / 422 L3 / 12 GN), 0043–0045 tầng tuân thủ SPEC-05 (controls, clause_controls, compliance_gaps, v_compliance_coverage, gen_compliance_tasks) — đối chiếu: docs/plan/14) · `src/lib` (db, auth, events(zod), queries, metrics, anychart (biểu đồ MỌI trường), admin (sổ đăng ký bảng quản trị + CSV), forms, offline, jobs, notify (event bus → hộp thư, luật cấu hình, auto-start quy trình)) · `src/app/api` (auth, events, data, actions ~60, series (+/any), exports (+crop-dossier), jobs, upload, notifications, admin/[table], import/csv, search, public/trace) · `src/components/panels/*` (màn theo vai) · `scripts/` (migrate, simulate) · `tests/`.
